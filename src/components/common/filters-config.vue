                ...
                >
                    ...
                    <div class="trees">
                      ...
                      >
                        ...
                        >
                          ...
                          <span class="num" v-if="node.label !== '热门期刊'"
                            >({{ data.doc_count }})</span
                          >
                        </span>
                      </el-tree>
                  ...
                  <div
                    v-else-if="
                      ...
                      (key === 'source' || key === 'source_formulation')
                    "
                  >
                  ...
                  <el-checkbox-group
                    v-model="checked[key]"
                    v-else
                    :class="{ yaowuhuaxiang: key === 'yaowuhuaxiang' }"
                  >
                    <el-checkbox
...
export default {
  ...
  computed: {
    ...
    filtersSourceFormulation() {
      return Store.state["Globalapproval"].filtersSourceFormulation;
    },
    nopms() {
  ...
  methods: {
    ...
    init() {
      ...
      for (const [key, val] of Object.entries(this.filters)) {
        ...
        this.filters1.forEach((item2) => {
          ...
          if (
            this.vuex_name === "Globalapproval" &&
            key === "source_formulation" &&
            item2.field === "source_formulation"
          ) {
            this.filters[key] = this.filters[key].map((v) => ({
              label: v.label,
              key: v.key,
            ...
            // 全球上市国家和地区按活性成分
            if (this.storeState.filtersSourceFormulation.length > 0) {
              this.filters[key].forEach((val, index) => {
                this.storeState.filtersSourceFormulation.forEach((item) => {
                  if (val.label === item.label) {
                    this.filters[key][index].checked = item.checked;
                  }
                });
    ...
    searchFilter() {
      ...
      if (this.vuex_name === "Globalapproval") {
        ...
        Store.commit(
          this.vuex_name + "/filtersSourceFormulation",
          this.filters["source_formulation"]
        );
      }
  ...
  watch: {
    ...
    searchLoading(val) {
      if (!val) {
        ...
        this.defaultExpanded = this.checked["results"];
        this.defaultExpanded2 = this.checked["leixingbm"];
        this.defaultExpanded2 = this.checked["type"];
        this.defaultExpanded2 = this.checked["dizhi"];
        this.defaultExpanded = this.checked["typeLevel"];

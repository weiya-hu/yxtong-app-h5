<template>
  <div class="la-optioned clearfix policies-optioned" :class="{
    'option-min-height': minHeight,
    'option-show': showHandler(),
  }">
    <!-- 保存条件 - 输入名称 弹出框 -->
    ...
    <div class="optioned main" v-if="
      Object.keys(storeState.putong).length>1||
    ...
    ">
      <!-- <span
        ...
        class="iconfont">&#xe615;</span> -->
      <!-- 关联查询搜索 -->
      <span class="options-label">检索条件：</span>
      <span v-if="
      ...
      " class="show-options glcx-option" title="关联查询">
        <span class="btn option">
          ...
          <i class="el-icon-close" @click="handleFourthCloseClick()"></i>
        </span>
      </span>
      <!-- 普通搜索 -->
      <span v-for="(option,key,index) in storeState.putong
      ? handlePutong(storeState.putong)
      :{}" :key="index" class="show-options pt-option" :title="getPutongCnameRight(option,key)"
        v-if="key!=='keywordsType'">
        ...
        <span class="btn option">
          ...
          <i class="el-icon-close" @click="handlePutongCloseClick(key)"></i>
        </span>
      </span>
      <!-- 高级搜索 -->
      <span v-for="(option,index) in storeState.conditions
      ...
      " class="show-options gj-option" :title="getGaojiCnameRight(option.value,option.name,option)">
        ...
        <span class="btn option">
          ...
          <i class="el-icon-close" @click="handleGaojiCloseClick(option)"></i>
        </span>
      </span>

      <!-- 条件筛选 -->
      <div class="optioned screen" v-if="storeState.shaixuan&&storeState.shaixuan.filter_condition">
        ...
        <span class="btn option" v-for="(option,index) in getShaixuan(
          ...
          <i class="el-icon-close" @click="handleShaixuanCloseClick(option.name)"></i>
        </span>
      </div>
      <!-- 保存 & 清空 -->
...
export default {
  props: {
    ...
    route1: {
      // 同route2，route3，含有可视化分析才绑定这三个属性（比如注册有list//qb，而其他大多数只有两个属性，route3不绑定）
      type: String,
      default: "",
    },
    route2: {

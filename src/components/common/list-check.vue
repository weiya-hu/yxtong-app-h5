<template>
  <div style="display: inline-block">
    <span class="action-btn" @click="handleActionShow()"><i class="iconfont">&#xe76d;</i> 显示</span>
...
export default {
  ...
  methods: {
    ...
    handleActionShow () {
      ...
      if (this.allBase) {
        window.ga("send","event", "button", "click", this.allBase.dbename+"_display");
        window._paq.push(['trackEvent', 'button', 'click', this.allBase.dbename+"_display", ])

      }else if(this.vuex_name == "Database" ){
        const {params} = this.$route;
        window.ga("send","event", "button", "click", params.dbname+"_display");
        window._paq.push(['trackEvent', 'button', 'click',  params.dbname+"_display", ])

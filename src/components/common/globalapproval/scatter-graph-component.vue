<template>
  <div class="scatter-graph-component">
    <div class="title-content" v-if="infoObj.zwformCount > 0">
      <div class="title-first">
        {{ infoObj.zwform }}
        <span class="tips">(当前{{ infoObj.zwformCount }}种剂型)</span>
      </div>
      <div class="title-second" v-if="infoObj.zwformCount > maxNodeCount">
        默认展示{{ maxNodeCount }}种剂型，查看更多请点击右侧导出
      </div>
    </div>
...
export default {
  ...
  watch: {
    infoObj(newVal, oldVal) {},
    scatterData(newVal, oldVal) {
      this.initScatterGraph(newVal);
    }
  },
  mounted() {},
  methods: {
    initScatterGraph(datas) {
      ...
      const mapNodeSize = (nodes, propertyName, visualRange) => {
        ...
        nodes.forEach(node => {
          node.size =
            node.text.length * this.fontSize > 6 * this.fontSize
              ? node.text.length * this.fontSize
              : 6 * this.fontSize;
        });
      };
      ...
      if (this.graph) {
        ...
        if (datas.length > 0) {
          datas.forEach(item => {
            ...
            if (item.child && item.child.length > 0) {
              tags = item.child.map(item2 => item.label + "-" + item2.label);
            }
            // level 0
            nodeData.nodes.push({
              count: parseInt(item.doc_count),
              id: item.label,
              key: item.key,
              name: item.label,
              text: item.label,
            ...
            item.child &&
              ...
              item.child.forEach(item2 => {
                // level 1
                nodeData.nodes.push({
                  count: parseInt(item2.doc_count),
                  id: item.label + "-" + item2.label,
                  key: item2.key,
                  name: `${item2.label}(${parseInt(item2.doc_count)})`,
                  tag: item2.label,
                  tags: [item.label],
                  text: `${item2.label}(${parseInt(item2.doc_count)})`,
                  level: 1,
                  lang: "en",
                  type: "son"
                });
                nodeData.edges.push({
                  source: item.label,
                  target: item.label + "-" + item2.label
                });
              });
          });
          this.totalNodeData = _.cloneDeep(nodeData);
        }
        ...
        showNodes = nodeData.nodes;
        showEdges = nodeData.edges;
      ...
      this.graph.on("node:click", e => {
        ...
        if (model.level === 0) {
          ...
          showNodes.forEach(snode => {
            ...
            item && this.graph.setItemState(item, "dark", false);
          ...
          let searchObj = this.$parent.$parent.searchObj,
            query = {};
          if (searchObj.keyword) query.name_eq = searchObj.keyword;
          let area = model.text.substring(0, model.text.indexOf("("));
          query.globalapprovalSource = JSON.stringify([
            {
              label: area,
              key: area,
              checked: 1
          ...
          query.filter_condition = JSON.stringify({
            zwform: [model.id.substring(0, model.id.indexOf("-"))]
          });
      ...
      this.graph.on("canvas:click", () => {
        ...
        if (highlighting) {
          ...
          setTimeout(() => {
            ...
            layoutController.layoutCfg.collideStrength = 0.6;
            layoutController.layoutCfg.alphaDecay = 0;
            layoutController.layoutCfg.edgeStrength = 1;

      ...
      <div class="item_content1">
        <el-table ref="ElTable"
          ...
          <el-table-column v-for="item in tableHead"
            ...
            <!-- 自定义表头 -->
            <template :slot="1 ? 'header':''"
              slot-scope="scope">
              <span>
                <NewProjectGuide  postion="列表" :newPosition="{top:-10,right:-20}" :onlyKey="item['new_id']" display="inline" compoentsName="字段" :initItem="item" :showMenban="item['is_show_new']">{{item.label}}</NewProjectGuide>
              </span>
                ...
                <span v-if="vueCheckListPms('Database', item.label)"
                  ...
                  <TdCountCheckOut v-else-if="item.readtype == '超链接(特殊域名)' && item.contentname != '下载'"
                    ...
                    <span>
                      ...
                      <span v-else
                        :title="(item.fieldmax?checkOne(item, scope.row):'')">
                        {{ overflowEclips(item.nodata_content,item.fieldmax) }}
                      </span>
                    </span>
                  </TdCountCheckOut>
                  <!-- 非下载 - 超链接(存储字段) -->
...
<script>
  ...
  import TdCountCheckOut from "@/components/common/tdCountCheckOut"

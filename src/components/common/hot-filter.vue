<template>
  ...
  <transition name="el-zoom-in-top" appear>
    <div class="hot-filter item" v-if="hasHotFilters" :class="{ 'abandon-click-method': nopms && nopms.rdss }">
      <span class="item-title">热点搜索</span>
        ...
        <div class="filter-lists_box">
          <div v-for="(key, index) in keys" :key="index">
            <el-checkbox-group v-model="checked[key]">
              <el-checkbox :disabled="nopms && nopms.rdss" v-for="item in hot_filed[key]" :label="item.key"
                :key="item.key" @change="handleCheckedChange(item.key, item.param)">
                <span>
                  <!-- {{item.label}} -->
                  <NewProjectGuide postion="热点" :onlyKey="item['new_id']" display="inline" compoentsName="条件筛选"
                    :initItem="item" :showMenban="item['is_show_new']">{{ item.label }}</NewProjectGuide>
                </span>
      ...
      <el-button class="control-filter" :disabled="nopms && nopms.rdss" size="mini" type="primary" @click="search">筛选
      </el-button>
    </div>
  </transition>
</template>
...
export default {
  props: {
    ...
    dbname: { // 没有定量分析不要绑定此属性！（可参考药品销售数据库示例，截止9月30日，暂时只有药品销售/药品中标/生物制品这三个数据库有定量分析）
      type: String,
      default: ''
    },
    route1: { // 同route2，route3，含有可视化分析才绑定这三个属性（比如注册有list/analy/qb，而其他大多数只有两个属性，route3不绑定）
      type: String,
      default: ''
    },
    route2: {
  ...
  methods: {
    init() {
      for (let key in this.hot_filed) {
        this.$set(this.checked, key, this.hot_checked == undefined ? new Array() : (this.hot_checked[key] || new Array()))
      }
    },
    //搜索
    search() {
    ...
    handleCheckedChange(key, param) {
      window.ga("send", "event", "option", "click", this.vuex_name + "_hot_condition_" + param + "_" + key)
      window._paq.push(['trackEvent', 'option', 'click', this.vuex_name + "_hot_condition_" + param + "_" + key,])

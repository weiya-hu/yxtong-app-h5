<template>
  <div class="la-date">
    <div class="title">
      ...
      <el-tooltip v-else-if="initItem.field == 'yxq' && initItem.label == '有效日期' && vuex_name == 'Qxss'" effect="light"
        placement="right">
    ...
    <div v-if="initItem.limit.length != 0" class="shortcut-date">
      <el-radio v-for="(date, index) in initItem.limit" v-model='kjdate' :label="index" :key="index" @change="update(0)"
        @click.native.prevent="handleRadioClick(index)" ref="radoo">最近{{ date.count }}{{ $t('unit.' +
            date.unit)
        }}({{ date.num }})</el-radio>
    </div>
    <div class="date-range">
      <el-date-picker type="date" v-model="dateval[0]" @change="update(1)" placeholder="选择日期" size="mini"
        format="yyyy-MM-dd" value-format="yyyyMMdd" clearable :picker-options="pickerOptionsStart" ref="popoverStart">
      </el-date-picker>
      <span>-</span>
      <el-date-picker type="date" v-model="dateval[1]" @change="update(1)" placeholder="选择日期" size="mini"
        format="yyyy-MM-dd" value-format="yyyyMMdd" clearable :picker-options="pickerOptionsEnd" ref="popoverEnd">
      </el-date-picker>
    </div>
  </div>
</template>

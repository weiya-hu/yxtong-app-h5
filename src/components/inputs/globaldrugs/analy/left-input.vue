<template>
  <div class="la-input">
    <div class="title">
      <div class="flex">
        <label class="title-label">
          <NewProjectGuide postion="基础" display="inline" :newPosition="{ right: initItem.statement ? -40 : -26 }"
            :onlyKey="initItem['new_id']" compoentsName="检索功能" :initItem="initItem"
            :showMenban="initItem['is_show_new']">
            {{ initItem.label }}</NewProjectGuide>
        </label>
        <!-- <label class="title-label">{{initItem.label}}</label> -->
        <el-tooltip v-if="initItem.field == 'barmc' && initItem.label == '企业名称' && vuex_name == 'Qxba'" effect="light"
          placement="right">
    ...
    <inputPopover :value="value">
      <!-- 无自动提示的输入框 - （普通搜索）受理号 -->
      <el-input :data-name="initItem.field" v-model="value" @keyup.native.enter="getEnterKey()"
        @input="resetValue(value)" :placeholder="initItem.placeholder" clearable>
      </el-input>
    </inputPopover>
  </div>
</template>

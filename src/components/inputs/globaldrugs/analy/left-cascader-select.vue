<template>
  <div class="la-cascader">
      ...
      <div class="flex">
        <label class="title-label">
          <NewProjectGuide postion="基础" :onlyKey="item['new_id']" :newPosition="{ right: item.statement ? -40 : -26 }"
            display="inline" compoentsName="检索功能" :initItem="item" :showMenban="item['is_show_new']">{{ item.label }}
          </NewProjectGuide>
          <!-- {{item.label}} -->
        </label>
        ...
        <el-tooltip v-else effect="light" placement="right"
          content="ATC分类，即解剖学治疗学及化学分类系统。一个药品可能同时对应多个分类，默认为检索药品的主分类，勾选多分类时可检索多个。">
          <span class="section-ico-qa"></span>
        </el-tooltip>
      </div>
    ...
    <div>
      <el-cascader class="la-advenced-cascader" :placeholder="item.placeholder" :options="options"
        :props="cascaderConfig" @change="updateValue" v-model="value" separator=">" filterable change-on-select
        clearable ref="popoverCascader"></el-cascader>
    </div>
  </div>
</template>
<script>
export default {
  ...
  created() {
    ...
    if (this.vuex_name) {
      this.storeState.queryAnaly.forEach(e => {
  ...
  methods: {
    changeClassify() {
      let query = this.storeState.queryAnaly
      _.forEach(query, (item) => {
        if (item.name === 'atc') {
          item.atcs_sel = this.atcs_sel
        };
      })
      console.log(this.storeState.queryAnaly)
    },
    updateValue(update_value) {
      let last_index = update_value.length - 1
      let last_value = update_value[last_index]
      let value = { opt: 1, param: { name: 'atc', value: last_value, show: update_value, atcs_sel: this.atcs_sel, label: this.storeState.match_putongAnaly.atc, type: "fullselect" } }
      //opt=1增加条件或者替换已有条件
      if (value.opt && value.opt == 1) {
        if (this.storeState.queryAnaly.length == 0) {
          this.storeState.queryAnaly.push(value.param)
        } else {
          for (let i = 0, l = this.storeState.queryAnaly.length; i < l; i++) {
            if (value.param.name == this.storeState.queryAnaly[i].name) {
              // 需要判断last_value值存在才去重新赋值，否则的话直接删除当前的param，不然会传给后端一个undefind值导致筛选不出数据
              if (value.param.value) {
                this.storeState.queryAnaly[i] = value.param;
              } else {
                this.storeState.queryAnaly.splice(i, 1)
              }
              break;
            } else {
              if (i == l - 1) {
        ...
        this.storeState.queryAnaly = value
      }

...
export default {
  ...
  created() {
    if (Store.state[this.vuex_name].queryStrAnaly) {
      let query = JSON.parse(Store.state[this.vuex_name].queryStrAnaly).find(item => item.name == this.item.name)
  ...
  methods: {
    saveData() {
      if (this.checkData) {
        ...
        _.remove(Store.state[this.vuex_name].queryAnaly, n => {

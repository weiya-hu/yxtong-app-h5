<template>
  <div class="cpbj-box">
    ...
    <div class="chart-boxs">
      ...
      <!-- 治疗领域布局 -->
      <div class="chart-box zllyWord">
        <div class="chart-title">治疗领域布局</div>
        <div class="echarts" id="zllyWord">
          <div class="no-data">
            <img src="/static/imgs/trz/no_data.png" alt="" />
            <div class="text">暂无相关数据</div>
          </div>
    ...
    <Target :countries="'国外'" :qiyemingcheng="qiyemingcheng"></Target>
  </div>
</template>
...
export default {
  ...
  methods: {
    wordResize() {
      this.$nextTick(() => {
        this.echartsResize("gjfbWord");
        this.echartsResize("cplbWord");
        this.echartsResize("yplxWord");
        this.echartsResize("xmjdWord");
        this.echartsResize("zllyWord");
      });
    },
    echartsResize(id) {
        ...
        .then((res) => {
          ...
          if (res.data.code === 200 && res.data.data) {
            ...
            this.$nextTick(() => {
              ...
              this.cplbWordConfigure(res.data.data.leibie);
              this.yplxWordConfigure(res.data.data.leixing);
              this.xmjdWordConfigure(res.data.data.phase);
              this.zllyWordConfigure(res.data.data.indication);
            });
          }
        })
        .catch((err) => {
    ...
    worldConfigure(worldData) {
      ...
      let myChart = Echarts.init(
        document.getElementById("gjfbWord"),
        "yaozh_theme"
      );
      myChart.setOption({
        toolbox: {
    ...
    // 绘制药品类型堆叠图
    cplbWordConfigure(cplbWordData) {
      var _len = cplbWordData.length;
      var yAxis_data = [];
      var series_data = [[]];
      var series_data_result = [];
      var legend_data_len = this.cplbWordArr.length;
      if (_len > 0) {
        for (var k = 0; k < _len; k++) {
          yAxis_data.push(cplbWordData[k].label);
        }

        for (var j = 0; j < legend_data_len; j++) {
          let tempArr = [];
          for (var l = 0; l < cplbWordData.length; l++) {
            tempArr.push(
              cplbWordData[l].children[j] !== undefined
                ? cplbWordData[l].children[j].doc_count
                : 0
            );
          }
        ...
        let myChart = Echarts.init(
          document.getElementById("cplbWord"),
          "yaozh_theme"
        );
        myChart.setOption({
          legend: {
    ...
    // 绘制药品分类堆叠图
    yplxWordConfigure(cplbWordData) {
      var _len = cplbWordData.length;
      var yAxis_data = [];
      var series_data = [[]];
      var series_data_result = [];
      ...
      if (_len > 0) {
        ...
        let myChart = Echarts.init(
          document.getElementById("yplxWord"),
          "yaozh_theme"
        );
        myChart.setOption({
          legend: {
    ...
    xmjdWordConfigure(data) {
      ...
      let myChart = Echarts.init(
        document.getElementById("xmjdWord"),
        "yaozh_theme"
      );
      myChart.setOption({
        tooltip: {
    ...
    zllyWordConfigure(data) {
      ...
      for (var k = 0; k < data.length; k++) {
        xAxis_data.push(data[k].label);
        for (var j = 0; j < this.cplbWordArr.length; j++) {
      ...
      let myChart = Echarts.init(
        document.getElementById("zllyWord"),
        "yaozh_theme"
      );
      myChart.setOption({
        legend: {

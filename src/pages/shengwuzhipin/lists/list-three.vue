      ...
      <el-table
                 ...
                 <el-table-column
                  ...
                  <template slot-scope="scope">
                    <div v-if="item.prop === 'pihao'" :title="scope.row.pihao">
                      <a target="_blank" :href="'/pqf/'+scope.row.id" class="cl-blue">{{ scope.row.pihao }}</a>
                    </div>
                    <div v-if="item.prop === 'name'" :title="scope.row.name">
                      {{ scope.row.name }}
                    </div>
        ...
        <el-table-column
         v-for="item in tableHead"
          ...
          :label="item.label">
          <template slot-scope="scope">
            <div v-if="item.prop === 'qiye'" :title="scope.row.qiye" class="extensible-td cl-blue" @click="getExpandRes(scope.row.qiye,item.prop,scope)">
              {{ scope.row.qiye }}
            </div>
            <div v-if="item.prop === 'piliang'" :title="scope.row.piliang">
              {{ scope.row.piliang }}
...
<script>
  ...
  const defaultFiled = tablehead.shengwuzhipin.qy
  const expandFiled = tablehead.shengwuzhipin.qyExpand
  export default{
    components: {
      LoadingGif,
      List,
    ...
    computed: {
      ...
      ...mapState({
        ...
        res: state => state.Shengwuzhipin.res3,
        ...
        param: state => state.Shengwuzhipin.qy_param, // 附加参数(存于vuex内)
    ...
    methods: {
      ...
      // 定量分析 ---> 点击企业分析
      showAnaly (qymc) {
        this.$store.commit('Shengwuzhipin/dlfx_nowNav', '企业分析')
        this.$store.commit('Shengwuzhipin/dlfx_searchValQy', qymc);
        this.$store.commit('Shengwuzhipin/dlfx_searchListQy', [
          {label: qymc, value: qymc},
        ]);
      },
      openSaleDetail(scope){
        return '/api/pqf/mxbboutdata?qiye='+encodeURIComponent(scope.row.qiye)+Object.keys(this.search_api_param).reduce((total, curr) => {
      ...
      handleRequest (props, order) {
        Store.dispatch("Shengwuzhipin/getGroupRes",{path:'/api/pqf/byqiye',params:this.expandParams}).then(res=>{
      ...
      sortChange (column, prop, order) {
        ...
        Store.commit('Shengwuzhipin/order', Object.assign({}, this.$store.state.Shengwuzhipin.order, {qiye: sort ? sort : null}))
        Store.dispatch('Shengwuzhipin/nomalSearch',{params:this.param})
      },
      // 内部排序
      ...
      getExpandRes (qiye,name,scope) {
        window.ga("send", "event", "tab", "click", this.allBase.dbename+"_byqiye_"+name)
        window._paq.push(['trackEvent', 'tab', 'click',this.allBase.dbename+"_byqiye_"+name,])

        this.expandParams.qiye = qiye

<template>
  <div v-if="!loading">
    ...
    <span v-if="itemFather.special_img == '1'" class="img-wrap">
      ...
      <template v-if="isCas">
        <img
          :title="getKeys(item) + '，点击图片放大'"
          style="cursor:pointer"
          @click="handleMaginfyCASImg(item[getKeys(item)], index)"
          :src="item[getKeys(item)]"
          v-for="(item, index) in onlyImg"
          :height="itemFather.imgSize + 'px'"
          :key="index"
          @error="picLoadError($event, index)"
        />
      </template>
      <img
        v-else
    ...
    <span v-else-if="itemFather.num_down == '1'">
      <span
        v-html="
          overflowEclips(
            vueFormulationsFormatter(
              scopeRow ? scopeRow : itemFather.nodata_content
            ),
            itemFather.fieldmax
          )
        "
...
export default {
  ...
  data() {
    return {
      errorImgList: [],
      errorImg: false,
      imgSrcNeed: this.scopeRowAll[this.itemFather.parameter]
        ? this.scopeRowAll[this.itemFather.parameter]
        : this.itemFather.nodata_content || "undefined"
    };
  },
  computed: {
    ...mapState({
    ...
    imgSrckaifaType() {
      return this.scopeRowAll[this.itemFather.callbackfield]
        ? this.scopeRowAll[this.itemFather.callbackfield]
        : this.itemFather.nodata_content || "undefined";
    },
    imgSrcHxs() {

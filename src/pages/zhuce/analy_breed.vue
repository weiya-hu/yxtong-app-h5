<template>
  <div class="analy-wrapeer">
    ...
    <!-- 顶部统计 -->
    <div class="chart_top">
      <LoadingGif :className="'top-LoadingBreed'" :loadFlag="topLoadingBreed" v-if="topLoadingBreed">
      </LoadingGif>
      <div class="newtotals">
        ...
        <div class="newtotals-scroll">
          <div class="newtotals-scroll-inner">
            <ul class="total-item">
              <li class="li1">申报数量</li>
              <li class="li2">
                总计
                <span>{{ basic.total[0] }}</span>个品种
              </li>
              <li class="li3">报临床{{ basic.total[1] }}个品种</li>
              <li class="li3">报生产{{ basic.total[2] }}个品种</li>
            </ul>
            <ul class="total-item">
              <li class="li1">申报最多年份</li>
              <li class="li2">
                <span>{{ basic.shenbaonum[0] }}年</span>最多
              </li>
              <li class="li3">共{{ basic.shenbaonum[1] }}个品种</li>
            </ul>
            <ul class="total-item">
              <li class="li1">已批准品种数</li>
              <li class="li2">
                总计
    ...
    <div class="chart_bottom">
      <div class="chart-boxes">
        <!-- 自定义加载界面 -->
        <LoadingGif :className="'top-LoadingBreed'" style="height: calc(100vh - 223px);" :loadFlag="analyLoadingBreed"
          v-if="analyLoadingBreed"></LoadingGif>
...
export default {
  ...
  data() {
    return {
      ...
      pizhunBreed2: [],
      // 申报趋势
      shenbaoshuliangBreed2: [],
      // 申报数量
      shenbaobanliBreed2: [],
      // 申请类型
      shenqingleixingBreed2: [],
      // 药品类型
      yaopinleixingBreed2: [],
      // 审评结论
      xulieBreed2: [],
      // 办理状态
      guifanzhuangtaizhongwenBreed2: [],
      // ATC分类
      atcBreed2: [],
      // 剂型
      guifanjixingBreed2: [],
      // 企业排名
      guifanqiyezhongbiaoBreed2: [],
      // 申报地区
      shengfenBreed2: [],
      sbqs_line: {
        echarts: null,
  ...
  computed: {
    ...mapState({
      paramBreed: state => state.Zhuce.paramBreed,
      // 基础数量统计
      baseBreed: state => state.Zhuce.baseBreed,
      // 申报趋势
      pizhunBreed: state => state.Zhuce.pizhunBreed,
      // 申报趋势
      shenbaoshuliangBreed: state => state.Zhuce.shenbaoshuliangBreed,
      // 申报数量
      shenbaobanliBreed: state => state.Zhuce.shenbaobanliBreed,
      // 申请类型
      shenqingleixingBreed: state => state.Zhuce.shenqingleixingBreed,
      // 药品类型
      yaopinleixingBreed: state => state.Zhuce.yaopinleixingBreed,
      // 审评结论
      xulieBreed: state => state.Zhuce.xulieBreed,
      // 办理状态
      guifanzhuangtaizhongwenBreed: state => state.Zhuce.guifanzhuangtaizhongwenBreed,
      // ATC分类
      atcBreed: state => state.Zhuce.atcBreed,
      // 剂型
      guifanjixingBreed: state => state.Zhuce.guifanjixingBreed,
      // 企业排名
      guifanqiyezhongbiaoBreed: state => state.Zhuce.guifanqiyezhongbiaoBreed,
      // 申报地区
      shengfenBreed: state => state.Zhuce.shengfenBreed,
      topLoadingBreed: state => state.Zhuce.topLoadingBreed,
      analyLoadingBreed: state => state.Zhuce.analyLoadingBreed,
  ...
  watch: {
    ...
    baseBreed(newVal) {
      this.diffSearchData();
      Store.commit('Zhuce/chengbanriqiTypeBreed', this.selectYear);
      if (typeof newVal != "undefined") {
        this.basic = newVal;
        this.basicBreed2 = newVal;
      }
    },
    shenbaoshuliangBreed(newVal) {
      this.pizhunBreed2 = this.pizhunBreed;
      this.shenbaoshuliangBreed2 = newVal;
      if (this.$route.path == "/zhuce/analy_breed") {
        this.dealSbqsLine(this.pizhunBreed, newVal);
      }
    },
    shenbaobanliBreed(newVal) {
      this.shenbaobanliBreed2 = newVal;
      if (this.$route.path == "/zhuce/analy_breed") {
        this.dealSbqsBar(newVal);
      }
    },
    shenqingleixingBreed(newVal) {
      this.shenqingleixingBreed2 = newVal;
      if (this.$route.path == "/zhuce/analy_breed") {
        this.dealSqlxBar(newVal);
      }
    },
    yaopinleixingBreed(newVal) {
      this.yaopinleixingBreed2 = newVal;
      if (this.$route.path == "/zhuce/analy_breed") {
        this.dealYplxBar(newVal);
      }
    },
    xulieBreed(newVal) {
      this.xulieBreed2 = newVal;
      if (this.$route.path == "/zhuce/analy_breed") {
        this.dealSpjlPie(newVal);
      }
    },
    guifanzhuangtaizhongwenBreed(newVal) {
      this.guifanzhuangtaizhongwenBreed2 = newVal;
      if (this.$route.path == "/zhuce/analy_breed") {
        this.dealBlztBar(newVal);
      }
    },
    atcBreed(newVal) {
      this.atcBreed2 = newVal;
      if (this.$route.path == "/zhuce/analy_breed") {
        this.dealYlflPie(newVal);
      }
    },
    guifanjixingBreed(newVal) {
      this.guifanjixingBreed2 = newVal;
      if (this.$route.path == "/zhuce/analy_breed") {
        this.dealJxBar(newVal);
      }
    },
    guifanqiyezhongbiaoBreed(newVal) {
      this.guifanqiyezhongbiaoBreed2 = newVal;
      if (this.$route.path == "/zhuce/analy_breed") {
        this.dealQypmBar(newVal);
      }
    },
    shengfenBreed(newVal) {
      this.shengfenBreed2 = newVal;
      if (this.$route.path == "/zhuce/analy_breed") {
    ...
    $route(newVal, oldVal) {
      ...
      //   this.loadData();
      // }
      // if (this.$route.path === "/zhuce/analy_breed" && !this.basic) {
      //   this.loadData();
      // }
      // if (this.$route.path === "/zhuce/analy_breed") {
  ...
  methods: {
    async selectChange(chengbanriqiType) {
      Store.commit('Zhuce/chengbanriqiTypeBreed', this.selectYear);
      Store.commit('Zhuce/paramBreed', { ...this.paramBreed, chengbanriqiType });
      let params = _.assign(this.paramBreed, { chengbanriqiType }),
        axiosList = [
          Store.dispatch("Zhuce/getPizhunBreed", { ...params, trend_subset: 0 }),
          Store.dispatch("Zhuce/getShenbaoshuliangBreed", { ...params, trend_subset: 1 })
        ],
        res = await Promise.all(axiosList);

      if (res[0].data.code === 200 && res[0].data.data) {
        Store.commit('Zhuce/pizhunBreed', res[0].data.data);
      }
      if (res[1].data.code === 200 && res[1].data.data) {
        Store.commit('Zhuce/shenbaoshuliangBreed', res[1].data.data);
        this.dealSbqsLine(this.pizhunBreed, this.shenbaoshuliangBreed);
      }
    },
    getImgName(i) {
      let title = i ? this.title[i] : this.title[this.openIndex];
      return `药智数据企业版-${document.title}-智能分析-按品种统计-${this.title[i]}`;
    },
    initLayout() {
    ...
    loadData: _.debounce(function () {
      Store.dispatch("Zhuce/getKshRes_Breed").then(res => {
        this.update();
      });
    }, 10),
    ...
    update() {
      ...
      this.dealSbqsLine(this.pizhunBreed, this.shenbaoshuliangBreed);
      this.dealSbqsBar(this.shenbaobanliBreed);
      this.dealSqlxBar(this.shenqingleixingBreed);
      this.dealYplxBar(this.yaopinleixingBreed);
      this.dealSpjlPie(this.xulieBreed);
      this.dealBlztBar(this.guifanzhuangtaizhongwenBreed);
      this.dealYlflPie(this.atcBreed);
      this.dealJxBar(this.guifanjixingBreed);
      this.dealQypmBar(this.guifanqiyezhongbiaoBreed);
      this.dealSbdqMap(this.shengfenBreed);
    ...
    dealSbqsLine(pizhun, shenbaoshuliang) {
      var xAxis_data = [];
      var series_data = [[], []];
      console.log(pizhun, shenbaoshuliang);
    ...
    dealSqlxBar(shenqingleixing) {
      ...
      var series_data = [[]];
      var series_data_result = [];
      var legend_data = this.yaopinleixingBreed.map(item => item.label);
    ...
    dealYplxBar(yaopinleixing) {
      ...
      var series_data = [[]];
      var series_data_result = [];
      var legend_data = this.shenqingleixingBreed.map(item => item.label);
      var legend_data_len = legend_data.length;
      // 数据全为0，显示nodata

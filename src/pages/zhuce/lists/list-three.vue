        ...
        <el-table-column type="expand" width="1">
          <div type="expand" slot-scope="props" class="table-expand">
                ...
                >
                  <template slot-scope="scope">
                    <div :title="scope.row[item.prop]">
                      ...
                      <div v-if="item.prop === 'name'">{{ scope.row.name }}</div>
                      <!-- <div v-if="item.prop === 'zhucefenlei'">
                      {{ scope.row.zhucefenlei }}
                      </div>-->
                      <div v-if="item.prop === 'shenqingleixing'">{{ scope.row.shenqingleixing_1}}</div>
                      <div v-if="item.prop === 'chengbanriqi'">{{ scope.row.chengbanriqi }}</div>
                      <div v-if="item.prop === 'zhuangtai'">{{ scope.row.zhuangtai }}</div>
                      <div v-if="item.prop === 'zhuangtaidate1'">{{ scope.row.zhuangtaidate1 }}</div>
                      <div v-if="item.prop === 'jielun'">{{ scope.row.jielun }}</div>
                    </div>
                  </template>
                </el-table-column>
              </el-table>
            ...
            <div v-else-if="tableType==='two'">
              <el-table :data="props.row.second[scope].res" fixed ref="expandtable" style="width: 100%">
                <el-table-column
          ...
          <template :slot="item.tips ? 'header':''" slot-scope="scope">
            <span>
              ...
              <el-tooltip class="item" effect="zhuce" placement="right">
                <div
                  slot="content"
                  v-if="item.prop == 'pingzhongnum'"
                >{{helpDesc.pingzhongnum}}</div>
                <i class="el-icon-question cl-green" style="margin-left:4px;"></i>
              </el-tooltip>
            </span>
          </template>
          <template slot-scope="scope">
            <div :title="item.prop==='zdsbpz'||item.prop==='zxzt'?'':scope.row[item.prop]">
              <div v-if="item.prop === 'guifanqiyezhongbiao'">
                <!-- <span
                  @click="handleRss(scope.row.rss&&scope.row.rss.qiye===1,4,scope.row.shoulihao,scope.row.name,scope.row.guifanqiyezhongbiao)"
                  class="row-absolute"
                  :class="scope.row.rss&&scope.row.rss.qiye===1?'row-absolute-ok':'row-absolute-no'"
                >{{scope.row.rss&&scope.row.rss.qiye===1?'已订':'订阅'}}</span> -->
              ...
              <div
                v-else-if="item.prop==='zdsbpz'"
                :class="{'extensible-num-td': Number(scope.row.zdsbpz.count)}"
                @click="otherGetExpand(scope.row, item.prop, 'name','guifanname_eq',scope)"
                :title="scope.row.zdsbpz.name"
              >
                <span v-if="scope.row.zdsbpz.name">
                  {{scope.row.zdsbpz.name}}
                  <span>
                    (
                    <span class="num-text">{{scope.row.zdsbpz.count}}</span>)
                  </span>
                </span>
                <span v-else>/</span>
              </div>
...
const defaultFiled = tablehead.zhuce.qy
const expandFiled = tablehead.zhuce.qyExpand
const expandFiledTwo = tablehead.zhuce.qyExpandTwo
...
export default {
  ...
  data() {
    return {
      ...
      tableType: null,
      getRowKeys(row) {
        return row.guifanqiyezhongbiao
      },
  ...
  computed: {
    ...
    ...mapState({
      ...
      res3: state => _.keys(state.Zhuce.res3).length ? state.Zhuce.res3 : [],
      ...
      param: state => state.Zhuce.qy_param, // 附加参数(存于vuex内)
  ...
  methods: {
    ...
    sortChange(column, prop, order) {
      ...
      Store.commit('Zhuce/order', Object.assign({}, this.$store.state.Zhuce.order, { qiye: sort ? sort : null }))
      Store.dispatch('Zhuce/nomalSearch', { params: this.param })
    },
    ...
    getExpandRes(qiyemingcheng, groupname, num, row, scope) {
      console.log(num);
      window.ga("send", "event", "tab", "click", this.allBase.dbename + "_qiyegroup_" + groupname)
      window._paq.push(['trackEvent', 'tab',"click",this.allBase.dbename + "_qiyegroup_" + groupname,  ])
    ...
    otherGetExpand(row, key, item_key, post_key, scope) {
      ...
      console.log(row, key, item_key, post_key, scope);
    ...
    handleRequest(props, order) {
      ...
      Store.dispatch("Zhuce/getGroupRes", { path: '/api/zhuce/qiyelist', params, row: this.currentRow, type: 1 }).then(res => {
    ...
    toggleRowExpansion(row, column, cell, event) {
      ...
      let newColum = ['zdsbpz', 'zhuangtaidate1', 'yaopin_high']
      ...
      if (num) {
        //前三个字段用表格1
        let tableTypeArr = ['yaopin_high', 'zhuangtaidate1', 'zdsbpz']
        ...
        if (column.id == this.expands.expanCell && isOneRow) {
          ...
          this.expands.expandsrow.push(row.guifanqiyezhongbiao)
          this.expands.element.removeClass('active')

        ...
        >
          <template slot-scope="scope">
            <div :title="scope.row[item.prop]">
              <div
                v-if="item.prop === 'qyfx'"
                @click="showAnaly(scope.row.qiye)"
              >
                <router-link
                  target="_blank"
                  class="cl-blue"
...
const defaultFiled = tablehead.yyxs.qy;
export default {
  components: {
    LoadingGif,
    List,
  ...
  computed: {
    ...
    ...mapState({
      ...
      res3: state =>
        _.keys(state.Yyxs.res3).length ? state.Yyxs.res3.res : [],
      ...
      param: state => state.Yyxs.qy_param, // 附加参数(存于vuex内)
  ...
  methods: {
    ...
    // 定量分析 ---> 点击企业分析
    showAnaly(qymc) {
      this.$store.commit("Yyxs/dlfx_nowNav", "企业分析");
      this.$store.commit("Yyxs/dlfx_searchValQy", qymc);
      this.$store.commit("Yyxs/dlfx_searchListQy", [
    ...
    getExpandRes(drugsname, groupname, num, label, scope) {
      if (num) {
        ...
        this.expandParams.qiye = drugsname;
    ...
    toggleRowExpansion(row, column, cell, event) {
      ...
      window.ga(
        ...
        "txt",
        "click",
        "yyxs_" + "_byqiye_" + column.property
      );
      window._paq.push([
        "trackEvent",
        "txt",

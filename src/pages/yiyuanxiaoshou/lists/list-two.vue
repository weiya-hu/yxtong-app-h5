        ...
        >
          <template slot-scope="scope">
            <div :title="scope.row[item.prop]">
              <div
                v-if="item.prop === 'ypfx'"
                @click="showAnaly(scope.row.guifanname)"
              >
                <router-link
                  target="_blank"
                  class="cl-blue"
...
const defaultFiled = tablehead.yyxs.yptym;
export default {
  components: {
    LoadingGif,
    List,
  ...
  computed: {
    ...
    ...mapState({
      ...
      res2: (state) =>
        _.keys(state.Yyxs.res2).length ? state.Yyxs.res2.res : [],
      ...
      param: (state) => state.Yyxs.name_param, // 附加参数(存于vuex内)
  ...
  methods: {
    ...
    getExpandRes(drugsname, groupname, num, label, scope) {
      if (num) {
        ...
        this.expandParams.name = drugsname;
    ...
    toggleRowExpansion(row, column, cell, event) {
      ...
      window.ga(
        ...
        "txt",
        "click",
        "yyxs_" + "_byname_" + column.property
      );
      window._paq.push([
        "trackEvent",
        "txt",

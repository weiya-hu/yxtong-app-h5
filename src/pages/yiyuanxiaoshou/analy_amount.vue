      ...
      <div class="chart_bottom">
        ...
        <div class="chart-boxes">
          <template v-for="(item, i) in echartsList">
            <DbEcharts :title="i < 3 ? tempTitle[i] : item.title" :key="i" v-if="!(item.api == 'djfb_xszb')">
          ...
          <div class="echarts-items">
            <div class="echarts-item" v-for="(item, i) in echartsListOpen" :key="i" v-show="i === openIndex">
              <MEcharts :option="item.option" class="open-charts" :ref="'open-echarts' + i" :id="'open-echarts' + i"
                :width="echartsOpenWidth" :conditions="conditions" :height="700" :name="item.api" :atcsSel="atcsSel" />
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      tempTitle: [],
      titles: ["药品销售量排行", "品牌销售量排行", "企业销售量排行"],
      n: 0,
      kshResTotal: {},
      totalList: [
  ...
  watch: {
    ...
    kshRes(obj) {
      if (this.$route.path === "/yyxs/analy_amount") {
    ...
    $route(n, o) {
      ...
      if (this.$route.path === "/yyxs/analy_amount" && !this.kshRes.aggs) {
        this.loadData();
      }
      if (this.$route.path === "/yyxs/analy_amount") {
  ...
  methods: {
    async exportHandler(index, subIndex) {
      ...
      let outdata_column = `field=${title},amount=销售量（${unit}）`;
      // 特殊：医疗机构销售额
      if (index === 4) {
        moduleName = "医疗机构销售量";
        // 占比统计
        if (subIndex === 0) {
          ...
          outdata_column = `field=医院等级,amount=销售数据占比`;
        } else if (subIndex === 1) {
          outdata_action = "djfb_xse";
          // 数据统计
          outdata_column = `field=医院等级,amount=销售量（万片/粒/袋/支）`;
        }
      }
    ...
    loadData: _.debounce(function () {
      ...
      // 加载 aggs API的数量总计
      this.promise = Store.dispatch("Yyxs/getKshRes", 2).then((res) => {
        this.update(res);
        return res;
      });
      setTimeout(() => {

<template>
  <Main>
    ...
    <el-dialog class="dialog-saved" :visible.sync="savedPopVisible" :modal-append-to-body="false">
      ...
      <SavedPop @closeSavedPop="closeSavedPop" vuex_name="Ywhc2"></SavedPop>
    </el-dialog>
    ...
    <div slot="left">
      ...
      <!-- 保存的条件 -->
      <Saved vuex_name="Ywhc2" api_name="fzk1"></Saved>
      <!-- 历史搜索 -->
  ...
  export default{
    ...
    computed:{
      ...mapState({
        ...
        nomal_filed: state => state.Ywhc2.nomal_filed,
        advanced_filed: state => state.Ywhc2.advanced_filed,
        putong: state => state.Ywhc2.putong,
        extendCount: state => state.Ywhc2.extendCount,
        update_conf: state => state.Ywhc2.update_conf,
      })
    },
    filters: {
      // 给数字每三位数加一个逗号
    ...
    methods:{
      ...
      //获取普通搜索
      nomalSearch () {
        Store.dispatch('Ywhc2/nomalSearch')
      },
    ...
    mounted () {
      Store.dispatch("Ywhc2/getBaseInfo").then(() => {
        this.vueTogglePmsTooltip()
      })
      this.vueRouteFrom("Ywhc2")
    },
  }

    ...
    <div class="detail-list" v-else>
      <div class="detail-header">
        <div class="header-left">
          <div class="left-top">
            <span class="top-id" :title="data.inn">{{data.inn}}</span>
          </div>
        </div>
      </div>
      <div class="main">
        <slide-section  :title="'基础信息'">
          <div class="tb-wrap">
            <table class="tb-t">
              <tr>
                <td>化学名称</td>
                <td colspan="3" v-html="data.hxm"></td>
              </tr>
              <tr>
                <td>分子式</td>
              ...
              <tr>
                <td>药理作用</td>
                <td colspan="3">{{data.ylzy}}</td>
              </tr>
              <tr>
                <td>结构式</td>
                <td colspan="3">
                  <img :src="'http://zy.yaozh.com//wmf/' + data.luj  + '/' + data.enden + '.jpg'" @error="picLoadError" class="img-jiegoushi">
                </td>
              </tr>
            </table>
          </div>
                    ...
                    <td style="padding: 0px">
                      <table class="tb-t-in">
                        ...
                        <tr v-for="(fzk, index2) in item.fzk" :key="index2">
                          ...
                          <td>{{fzk.en}}</td>
                          <td>
                            <router-link tag="a" :to="'/fzk1/' + fzk.id" class="cl-blue limit-pinming">{{fzk.pinm}}</router-link>
                          </td>
                          <td>{{fzk.casn}}</td>
  ...
  export default {
    ...
    methods: {
      handleAxios () {
        window.Axios({
          method: 'get',
          url: '/api/ywfzk/' + this.$route.params.id,
          params: {
          ...
          if (res.data.code === 200 && res.data.data) {
            ...
            this.getExtendList('ywfzk',res.data.data.extendList)
    ...
    mounted() {
      Store.dispatch("Ywhc1/getBaseInfo")
    },
    activated() {

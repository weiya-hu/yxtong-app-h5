<template>
  <Main>
    ...
    <div slot="left">
      ...
      <!-- 保存的条件 -->
      <Saved vuex_name="Ywhc1" api_name="ywfzk"></Saved>
      <!-- 历史搜索 -->
    ...
    <div class="la-right">
      ...
      <section class="sec2">
        ...
        <div class="search-body-content">
          <div class="option_wrapper"><Optioned
            @showSavedPop="showSavedPop"
            @clearTheLeft="clearTheLeft"
            @clearLeft="clearLeft"
            vuex_name="Ywhc1"></Optioned>
  ...
  export default{
    ...
    computed:{
      ...mapState({
        ...
        helpInfo: state => state.Ywhc1.helpInfo,
        nomal_filed: state => state.Ywhc1.nomal_filed,
        advanced_filed: state => state.Ywhc1.advanced_filed,
        putong: state => state.Ywhc1.putong,
        extendCount: state => state.Ywhc1.extendCount,
        update_conf: state => state.Ywhc1.update_conf,
      })
    },
    filters: {
      // 给数字每三位数加一个逗号
    ...
    methods:{
      ...
      //获取普通搜索
      nomalSearch () {
        Store.dispatch('Ywhc1/nomalSearch')
      },
      // 点击左侧栏中的“高级搜索”触发的事件
      handleAdvancedClick () {
        if (!this.nopms.gjss) {
    ...
    mounted () {
      Store.dispatch("Ywhc1/getBaseInfo").then(() => {
        this.vueTogglePmsTooltip()
      })
      this.vueRouteFrom("Ywhc1")
    },
  }

    ...
    <List :loading="listLoading" :nodata="res3 && !res3.length">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div :title="scope.row[item.prop]">
              <template v-if="item.prop === 'activeingredientCounts'">
                <span>{{scope.row[item.prop]}}({{scope.row.counts}})</span>
              </template>
              <!-- 最早批准日期 -->
...
<script>
  ...
  const defaultFiled = tablehead.globalapproval.qymc
  ...
  export default{
    ...
    data () {
      return {
        ...
        tableHead:[],
        getRowKeys(row) {
          return row.qiyebianmatz
        },
        expandParams:{   //存分组查询的条件
          qiyebianmatz_eq: '', // 企业编码
          date_eq: '',
          pageSize: 20,
          page:1
        },
    ...
    computed: {
      ...
      ...mapState({
        ...
        res3: state => _.keys(state.Globalapproval.res3).length ? state.Globalapproval.res3 : [],
    ...
    methods: {
      ...
      handleRequest(props){
        Store.dispatch("Globalapproval/getGroupRes",{path:'/api/globalapproval/qiyelist',params:this.expandParams}).then(res=>{
      ...
      getExpandRes (groupname,date,num,scope) {
        ...
        if (Number(num)) {
          this.expandParams.qiyebianmatz_eq = scope.row.qiyebianmatz
      ...
      toggleRowExpansion(row, column, cell, event){
        ...
        if (num) {
          ...
          if (column.id == this.expands.expanCell && isOneRow) {
            ...
            this.expands.expandsrow.push(row.qiyebianmatz)
            this.expands.element.removeClass('active')

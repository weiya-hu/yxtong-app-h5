...
export default {
  ...
  methods: {
    getTreeData(treeData) {
      ...
      function getNodesCount(node) {
        node.forEach(item => {
          if (item.children && item.children.length > 0) {
            nodesCount += item.children.length;
            getNodesCount(item.children);
          }
        });
      }
      getNodesCount([treeData]);

      // size of the diagram
      var viewerWidth = $(".ssgj").width();
      var viewerHeight = $(".ssgj").height();
      ...
      if (nodesCount<=20) {
        minWidth = viewerWidth / 3;
        minHeight = viewerHeight / 3;
      } else if (nodesCount>20 && nodesCount<50) {
        minWidth = viewerWidth / 2;
        minHeight = viewerHeight / 2;
      } else if (nodesCount>=50 && nodesCount<80) {
        minWidth = viewerWidth / 1.5;
        minHeight = viewerHeight / 1.5;
      ...
      var diagonal = d3.svg.diagonal().projection(function(d) {
        if (nodesCount > _this.maxNodesCount) {
          return [d.y / 5, d.x];
        } else {
          return [d.y, d.x];
        }

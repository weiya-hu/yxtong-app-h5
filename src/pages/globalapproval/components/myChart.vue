...
const list = ['globalapproval_analy_guojiapaihang_', 'globalapproval_analy_shangshiqushi_', 'globalapproval_analy_pingzongpaihang_', 'globalapproval_analy_qiyepaihang_', 'globalapproval_analy_jixing_', 'globalapproval_analy_atc_']
export default {
  components: {
    LoadingGif
  },
  ...
  computed: {
    conditions() {
      return this.$store.state.Globalapproval.conditions
    },
    isNoAtc() {
      let atcs_sel = false
  ...
  methods: {
    ...
    changeMapType() {
      window.ga("send", "event", "button", "click", "globalapproval_analy_diqu_qiehuan")
      window._paq.push(['trackEvent', 'button', 'click', 'globalapproval_analy_diqu_qiehuan',])

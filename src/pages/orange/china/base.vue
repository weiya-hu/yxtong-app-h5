<template>
  <Main>
    ...
    <div slot="left">
      ...
      <!-- 条件筛选（普通/高级搜索之后的二次筛选） -->
      <Filters vuex_name="Orangebook"></Filters>
      <!-- 保存的条件 -->
      <Saved vuex_name="Orangebook" api_name="orangebook"></Saved>
      <!-- 历史搜索 -->
    </div>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      nomal_filed: (state) => state.Orangebook.nomal_filed,
      advanced_filed: (state) => state.Orangebook.advanced_filed,
      putong: (state) => state.Orangebook.putong,
      extendCount: (state) => state.Orangebook.extendCount,
      update_conf: (state) => state.Orangebook.update_conf,
    }),
    // 判断高级搜索旁边的new标签是否显示
    is_show_new() {
  ...
  methods: {
    ...
    //获取普通搜索
    nomalSearch() {
      Store.commit("Orangebook/conditions", []);
      //清空高级搜索条件和筛选项
      Store.commit("Orangebook/groups", {});
      Store.dispatch("Orangebook/nomalSearch");
    },
    // 点击左侧栏中的“高级搜索”触发的事件
    handleAdvancedClick() {
      if (!this.nopms.gjss) {
  ...
  mounted() {
    Store.dispatch("Orangebook/getBaseInfo").then(() => {
      this.vueTogglePmsTooltip();
    });
    this.vueRouteFrom("Orangebook");
  },
};
</script>

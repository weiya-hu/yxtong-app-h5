<template>
  <Main>
    ...
    <el-dialog class="dialog-saved" :visible.sync="savedPopVisible" :modal-append-to-body="false">
      ...
      <SavedPop @closeSavedPop="closeSavedPop" vuex_name="FdaProducts"></SavedPop>
    </el-dialog>
    ...
    <div slot="left">
      ...
      <!-- 条件筛选（普通/高级搜索之后的二次筛选） -->
      <Filters vuex_name="FdaProducts"></Filters>
      <!-- 保存的条件 -->
      <Saved vuex_name="FdaProducts" api_name="fdaproducts"></Saved>
      <!-- 历史搜索 -->
  ...
  export default{
    ...
    computed:{
      ...mapState({
        ...
        nomal_filed: state => state.FdaProducts.nomal_filed,
        advanced_filed: state => state.FdaProducts.advanced_filed,
        putong: state => state.FdaProducts.putong,
        extendCount: state => state.FdaProducts.extendCount,
        update_conf: state => state.FdaProducts.update_conf,
      })
    },
    filters: {
      // 给数字每三位数加一个逗号
    ...
    methods: {
      ...
      //获取普通搜索
      nomalSearch () {
        Store.commit("FdaProducts/conditions",[])
        //清空高级搜索条件和筛选项
        Store.commit('FdaProducts/groups',{})
        Store.dispatch('FdaProducts/nomalSearch')
      },
      // 点击左侧栏中的“高级搜索”触发的事件
      handleAdvancedClick () {
        if (!this.nopms.gjss) {
    ...
    mounted () {
      Store.dispatch("FdaProducts/getBaseInfo").then(() => {
        this.vueTogglePmsTooltip()
      })
      this.vueRouteFrom("FdaProducts")
    },
  }
</script>

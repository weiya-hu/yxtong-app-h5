<template>
  <Main>
    <!-- 保存条件 - 输入名称 弹出框 -->
    <el-dialog :visible.sync="savedPopVisible" :modal-append-to-body="false" width="500px">
      <div slot="title" class="dialog-header">
        保存条件
      </div>
      <SavedPop @closeSavedPop="closeSavedPop" vuex_name="Pmda"></SavedPop>
    </el-dialog>
    ...
    <div slot="left">
      ...
      <div class="advanced-search">
        <!-- 高级搜索 -->
        <span class="fr adva-icon" v-if="Object.keys(this.$store.state.Pmda.match_gaoji).length">
      ...
      <!-- 条件筛选（普通/高级搜索之后的二次筛选） -->
      <Filters vuex_name="Pmda"></Filters>
      <!-- 保存的条件 -->
      <Saved vuex_name="Pmda" api_name="pmda"></Saved>
      <!-- 历史搜索 -->
    </div>
  ...
  export default{
    ...
    computed:{
      ...mapState({
        ...
        nomal_filed: state => state.Pmda.nomal_filed,
        advanced_filed: state => state.Pmda.advanced_filed,
        putong: state => state.Pmda.putong,
        extendCount: state => state.Pmda.extendCount,
        update_conf: state => state.Pmda.update_conf,
      })
    },
    filters: {
      // 给数字每三位数加一个逗号
    ...
    methods: {
      ...
      //获取普通搜索
      nomalSearch () {
        Store.commit("Pmda/conditions",[])
        //清空高级搜索条件和筛选项
        Store.commit('Pmda/groups',{})
        Store.dispatch('Pmda/nomalSearch')
      },
      // 点击左侧栏中的“高级搜索”触发的事件
      handleAdvancedClick () {
        if (!this.nopms.gjss) {
    ...
    mounted () {
      Store.dispatch("Pmda/getBaseInfo").then(() => {
        this.vueTogglePmsTooltip()
      })
      this.vueRouteFrom("Pmda")
    },
  }
</script>

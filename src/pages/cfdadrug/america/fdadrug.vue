<template>
  <Main>
    ...
    <el-dialog class="dialog-saved" :visible.sync="savedPopVisible" :modal-append-to-body="false">
      ...
      <SavedPop @closeSavedPop="closeSavedPop" vuex_name="FdaDrug"></SavedPop>
    </el-dialog>
    ...
    <div slot="left">
      ...
      <!-- 条件筛选（普通/高级搜索之后的二次筛选） -->
      <Filters vuex_name="FdaDrug"></Filters>
      <!-- 保存的条件 -->
      <Saved vuex_name="FdaDrug" api_name="fdadrug"></Saved>
      <!-- 历史搜索 -->
    ...
    <div class="la-right">
      ...
      <section class="sec1 clearfix">
        ...
        <div class="sec1-right">
          ...
          <div class="top-update-wrap" v-if="update_conf.log">
            {{update_conf.log}}
          </div>
        </div>
      </section>
  ...
  export default{
    ...
    computed:{
      ...mapState({
        ...
        nomal_filed: state => state.FdaDrug.nomal_filed,
        advanced_filed: state => state.FdaDrug.advanced_filed,
        putong: state => state.FdaDrug.putong,
        extendCount: state => state.FdaDrug.extendCount,
        update_conf: state => state.FdaDrug.update_conf,
      })
    },
    filters: {
      // 给数字每三位数加一个逗号
    ...
    methods: {
      ...
      //获取普通搜索
      nomalSearch () {
        Store.commit("FdaDrug/conditions",[])
        //清空高级搜索条件和筛选项
        Store.commit('FdaDrug/groups',{})
    ...
    mounted () {
      Store.dispatch("FdaDrug/getBaseInfo").then(() => {
        this.vueTogglePmsTooltip()
      })
      this.vueRouteFrom("FdaDrug")
    },
  }
</script>

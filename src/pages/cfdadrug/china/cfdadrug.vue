<template>
  <Main>
    ...
    <el-dialog :visible.sync="dialogFormVisible" :modal-append-to-body="false" width="815px">
      ...
      <Advanced :dialogFormVisible="dialogFormVisible" vuex_name="CfdaDrug" route1="list" route2="analy"
        api_name="cfdadrug" @closeAdvancedDialog="closeAdvancedDialog" @clearLeft="clearLeft" ref="advanced"></Advanced>
    </el-dialog>
    ...
    <div slot="left">
      ...
      <template v-for="(item, index) in vueHandleInitPutong(nomal_filed)">
        <Normal vuex_name="CfdaDrug" api_name="cfdadrug" route1="list" route2="analy" :item="item" :key="index"
          ref="ileft"></Normal>
      </template>
      <!-- 普通搜索按钮 & 高级搜索入口 -->
      ...
      <!-- 热点筛选条件 -->
      <HotFilter vuex_name="CfdaDrug" route1="list" route2="analy"></HotFilter>
      <!-- 条件筛选（普通/高级搜索之后的二次筛选） -->
      <Filters vuex_name="CfdaDrug" route1="list" route2="analy"></Filters>
      <!-- 保存的条件 -->
      <Saved vuex_name="CfdaDrug" api_name="cfdadrug"></Saved>
      <!-- 历史搜索 -->
    ...
    <div class="la-right">
      ...
      <section class="sec1 clearfix">
        ...
        <!-- <span class="database-num" v-if="extendCount.shoulihao_count&&$route.path.split('/')[2]==='list'">{{extendCount.shoulihao_count | formatNumber}}</span> -->
      ...
      <section class="sec2">
        ...
        <div class="search-body-content">
          <Optioned minHeight @showSavedPop="showSavedPop" @clearTheLeft="clearTheLeft" route1="list" route2="analy"
            @clearLeft="clearLeft" vuex_name="CfdaDrug"></Optioned>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      nomal_filed: state => state.CfdaDrug.nomal_filed,
      advanced_filed: state => state.CfdaDrug.advanced_filed,
      putong: state => state.CfdaDrug.putong,
      extendCount: state => state.CfdaDrug.extendCount,
      update_conf: state => state.CfdaDrug.update_conf,
    }),
    // 判断高级搜索旁边的new标签是否显示
    is_show_new() {
  ...
  methods: {
    ...
    //获取普通搜索
    nomalSearch() {
      Store.commit("CfdaDrug/conditions", [])
      //清空高级搜索条件和筛选项
      Store.commit('CfdaDrug/groups', {})
      Store.dispatch('CfdaDrug/nomalSearch')
    },
    // 点击左侧栏中的“高级搜索”触发的事件
    handleAdvancedClick() {
      if (!this.nopms.gjss) {
  ...
  mounted() {
    Store.dispatch("CfdaDrug/getBaseInfo").then(() => {
      this.vueTogglePmsTooltip()
    })
    this.vueRouteFrom("CfdaDrug")
  },
}
</script>

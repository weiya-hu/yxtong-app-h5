<template>
  <div class="la-list zdyScrool_Bar">
    <List :loading="listLoading" :nodata="res2 && !res2.res">
      <div class="actions">
      ...
      >
        <el-table-column type="expand" width="1">
          <div type="expand" slot-scope="props" class="table-expand">
            ...
            <div v-else-if="expands.tableType === 'cj'">
              <el-table
          ...
          <template slot-scope="scope">
            ...
            >
              ...
              <div
                ...
                "
                @click="
                  getExpandRes(scope.row.name, item.prop, scope.row, scope)
                "
              >
...
const expandFiled = tablehead.cfdadrug.nameExpand;
const expandFiledTwo = tablehead.cfdadrug.nameExpandTwo;
export default {
  components: {
    List,
    Export,
  ...
  computed: {
    ...
    ...mapState({
      ...
      res2: state => (state.CfdaDrug.res2.res ? state.CfdaDrug.res2 : {}),
      ...
      param: state => state.CfdaDrug.name_param, // 附加参数(存于vuex内)
  ...
  methods: {
    ...
    objectSpanMethod({ row, column, rowIndex, columnIndex }) {
      ...
      if (columnIndex === 0) {
        if (rowIndex === 0) {
          if (
            arr.length === 1 ||
            arr[rowIndex].shengchanqiye !== arr[rowIndex + 1].shengchanqiye
          ) {
            return [1, 1];
          } else {
            let rowspan = 1;
            for (let a = rowIndex + 1; a < arr.length; a++) {
              if (arr[rowIndex].shengchanqiye === arr[a].shengchanqiye) {
                rowspan += 1;
              } else {
                break;
              }
          ...
          if (arr[rowIndex].shengchanqiye !== arr[rowIndex - 1].shengchanqiye) {
            return [1, 1];
          } else {
            return [0, 0];
          }
    ...
    getExpandRes(name, groupname, row, scope) {
      window.ga(
        "send",
        "event",
        "tab",
      ...
      if (num) {
        this.expands.tableType =
          groupname.indexOf("changjia") !== -1 ? "cj" : "other";
        this.expandParams.name2_eq = name;

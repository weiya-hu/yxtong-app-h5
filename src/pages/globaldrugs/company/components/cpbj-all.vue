<template>
  <div class="cpbj-box">
    ...
    <div class="chart-boxs">
      ...
      <!-- 治疗领域布局 -->
      <div class="chart-box zlly">
        <div class="chart-title">治疗领域布局</div>
        <div class="echarts" id="zlly">
          <div class="no-data">
            <img src="/static/imgs/trz/no_data.png" alt="" />
            <div class="text">暂无相关数据</div>
          </div>
    ...
    <Target :countries="'全部'" :qiyemingcheng="qiyemingcheng"></Target>
  </div>
</template>
...
export default {
  ...
  methods: {
    allResize() {
      this.$nextTick(() => {
        this.echartsResize("gjfb");
        this.echartsResize("cplb");
        this.echartsResize("yplx");
        this.echartsResize("xmjd");
        this.echartsResize("zlly");
      })
    },
    echartsResize(id) {
    ...
    getCpbjAll() {
      ...
      window
        .Axios({
          method: "get",
          url: "/api/globaldrugs/companyProducts",
          params: {
        ...
        .then((res) => {
          ...
          if (res.data.code === 200 && res.data.data) {
            ...
            if(this.allData.country.length == 0 && this.allData.indication.length == 0 && this.allData.leibie.length == 0 && this.allData.leixing.length == 0 && this.allData.phase.length == 0 && this.allData.drugnamePhase.res.length == 0) {
              this.$emit('hideCpbj', true)
            }
            this.$nextTick(() => {
              this.worldConfigure(res.data.data.country);
              this.cplbConfigure(res.data.data.leibie);
              this.yplxConfigure(res.data.data.leixing);
              this.xmjdConfigure(res.data.data.phase);
              this.zllyConfigure(res.data.data.indication);
            });
          }
        })
        .catch((err) => {
    ...
    worldConfigure(worldData) {
      ...
      console.log(max);
      let myChart = Echarts.init(document.getElementById("gjfb"), "westeros");
      myChart.setOption({
        toolbox: {
          top: 20,
          right: 20,
    ...
    // 绘制药品类型堆叠图
    cplbConfigure(cplbData) {
      var _len = cplbData.length;
      var yAxis_data = [];
      var series_data = [[]];
      var series_data_result = [];
      var legend_data_len = this.cplbArr.length;
      if (_len > 0) {
        for (var k = 0; k < _len; k++) {
          yAxis_data.push(cplbData[k].label);
        }

        for (var j = 0; j < legend_data_len; j++) {
          let tempArr = [];
          for (var l = 0; l < cplbData.length; l++) {
            tempArr.push(
              cplbData[l].children[j] !== undefined
                ? cplbData[l].children[j].doc_count
                : 0
            );
          }
        ...
        let myChart = Echarts.init(document.getElementById("cplb"), "westeros");
        myChart.setOption({
          legend: {
            type: "scroll",
            bottom: 0,
    ...
    yplxConfigure(cplbData) {
      ...
      if (_len > 0) {
        ...
        let myChart = Echarts.init(document.getElementById("yplx"), "westeros");
        myChart.setOption({
          legend: {
            type: "scroll",
            bottom: 0,
    ...
    xmjdConfigure(data) {
      ...
      let myChart = Echarts.init(document.getElementById("xmjd"), "westeros");
      myChart.setOption({
        tooltip: {
          trigger: "item",
          axisPointer: {
    ...
    zllyConfigure(data) {
      ...
      for (var k = 0; k < data.length; k++) {
        xAxis_data.push(data[k].label);
        for (var j = 0; j < this.cplbArr.length; j++) {
          series_data[j].push(data[k].children[j].doc_count);
        }
      }

      console.log(series_data);
      let myChart = Echarts.init(document.getElementById("zlly"), "westeros");
      myChart.setOption({
        legend: {
          type: "scroll",
          top: 0,

    ...
    <List :loading="listLoading" :nodata="res3 && !res3.length">
      <div class="actions">
        ...
        <Export
          ...
          :max_num="extendCount.companyCount"
          :defaultFiled="exportDefaultFiled"
          outdataAction="companyGroup"
        ></Export>
      </div>
      <el-table
        :data="res3"
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <div>
              <template v-if="item.prop === 'company_standard'">
                <div v-for="(text, i) in scope.row.company_standard" :key="i">
                  <router-link
      ...
      <div style="positon: relative">
        <el-pagination
          ...
          :total="extendCount.companyCount"
          :disabled="nopms.fy"
          :class="{ 'abandon-click-method': nopms.fy }"
        >
        </el-pagination>
      ...
      <alert-wrong ref="alertWrongThree"></alert-wrong>
    </List>
  </div>
</template>
...
const defaultFiled = tablehead.globaldrugs.company;
const expandFiled = tablehead.globaldrugs.companyExpand;
export default {
  components: {
    LoadingGif,
    List,
  ...
  computed: {
    ...
    ...mapState({
      ...
      res3: (state) =>
        _.keys(state.Globaldrugs.res3).length ? state.Globaldrugs.res3 : [],
      ...
      param: (state) => state.Globaldrugs.qyll_param, // 附加参数(存于vuex内)
  ...
  methods: {
    ...
    handleRequest(props) {
      Store.dispatch("Globaldrugs/getGroupRes", {
        path: "/api/globaldrugs/companyList",
    ...
    getExpandRes(name, num, scope) {
      ...
      if (Number(num)) {
        this.expandParams.company_standard = scope.row.company;

  ...
  <div class="la-list globaldrugs-list">
    <List :loading="listLoading" :nodata="res5 && !res5.length">
      <div class="actions">
        ...
        <span class="action-btn" :class="{ 'abandon-click-method': nopms && nopms.compare }" @click="goToContrast">
          <img
        ...
        <Export
          ...
          :max_num="extendCount.indicationCount"
          :defaultFiled="exportDefaultFiled"
          outdataAction="indicationGroup"
        ></Export>
      </div>
      <el-table
        :data="res5"
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <div>
              <template v-if="item.prop === 'indication_standard'">
                <span style="font-size: 14px">{{ scope.row[item.prop] }}</span>
              </template>
              <!-- 药物数量 -->
      ...
      <div style="positon: relative">
        <el-pagination
          v-if="extendCount.indicationCount"
          ...
          :total="extendCount.indicationCount"
          :disabled="nopms.fy"
          :class="{ 'abandon-click-method': nopms.fy }"
        >
        </el-pagination>
      ...
      <alert-wrong ref="alertWrongFive"></alert-wrong>
    </List>
  </div>
</template>
...
const defaultFiled = tablehead.globaldrugs.indication;
const expandFiled = tablehead.globaldrugs.indicationExpand;
export default {
  components: {
    LoadingGif,
    List,
  ...
  data() {
    return {
      ...
      tableHead: [],
      getRowKeys(row) {
        return row.indication_standard;
      },
      expandParams: {
        //存分组查询的条件
        indication_standard: "", // 企业名称
        phase: "", // 研发阶段
        pageSize: 20,
        page: 1,
      },
  ...
  computed: {
    ...
    ...mapState({
      ...
      res5: (state) =>
        _.keys(state.Globaldrugs.res5).length ? state.Globaldrugs.res5 : [],
      ...
      param: (state) => state.Globaldrugs.syzll_param, // 附加参数(存于vuex内)
  ...
  methods: {
    ...
    handleRequest(props) {
      Store.dispatch("Globaldrugs/getGroupRes", {
        path: "/api/globaldrugs/indicationList",
    ...
    sortChange(column, prop, order) {
      ...
      Store.commit(
        ...
        Object.assign({}, this.$store.state.Globaldrugs.order, {
          indicationGroup: sort ? sort : null,
        })
      );
      Store.dispatch("Globaldrugs/nomalSearch", { params: this.param });
    },
    ...
    getExpandRes(name, num, scope) {
      ...
      if (Number(num)) {
        this.expandParams.indication_standard = scope.row.indication_standard;
    ...
    toggleRowExpansion(row, column, cell, event) {
      ...
      if (num) {
        ...
        if (column.id == this.expands.expanCell && isOneRow) {
          ...
          this.expands.expandsrow.push(row.indication_standard);
          this.expands.element.removeClass("active");

      ...
      <el-table
              ...
              <el-table
                ...
                style="width: 100%">
                 <el-table-column
                  ...
                  :key="ix"
                  :sortable="item.sort"
                  :min-width="item.width || item.field_width || 100"
                  :label="item.label">
        ...
        <el-table-column
         v-for="item in tableHead"
          ...
          <template slot-scope="scope">
            <div :title="scope.row[item.prop]">
            <div v-if="item.prop === 'guifanqiyezhongbiao'" class="extensible-td cl-blue" @click="getExpandRes(scope.row.guifanqiyezhongbiao,scope)">
              {{ scope.row.guifanqiyezhongbiao }}
            </div>
            <div v-if="item.prop === 'count_zhongbiao'">
              {{ scope.row.count_zhongbiao }}
            </div>
            <div v-if="item.prop === 'count_name'">
              {{ scope.row.count_name }}
            </div>
...
<script>
  ...
  const expandFiled = tablehead.yaopinzhongbiao.qyExpand
  export default{
    components: {
      LoadingGif,
      List,
    ...
    computed: {
      ...
      ...mapState({
        ...
        param: state => state.Yaopinzhongbiao.qy_param,
    ...
    methods: {
      ...
      showGhAnaly (currVal) {
        let val = [currVal];
        Store.commit('Yaopinzhongbiao/qytop', val);
        Store.commit('Yaopinzhongbiao/qiyefx', val);
      },
      init () {
      ...
      handleRequest(props, order){
        Store.dispatch("Yaopinzhongbiao/getGroupRes",{path:'/api/yaopinzhongbiao/getbyqiye',params:this.expandParams}).then(res=>{
      ...
      sortChange (column, prop, order) {
        ...
        Store.commit('Yaopinzhongbiao/order', Object.assign({}, this.$store.state.Yaopinzhongbiao.order, {qiye: sort ? sort : null}))
        Store.dispatch('Yaopinzhongbiao/nomalSearch',{params:this.param})
      },
      // 内部排序
      ...
      getExpandRes (gfqyzb,scope) {
        this.expandParams.guifanqiyezhongbiao = gfqyzb
      ...
      toggleRowExpansion(row, column, cell, event){
        ...
        if (column.property != 'guifanqiyezhongbiao') {
          return
        }
        let isOneRow = false

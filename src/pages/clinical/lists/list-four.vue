    ...
    <List :loading="listLoading" :nodata="res4 && !res4.length">
      <div class="actions">
        ...
        <Export
          ...
          vuex_name="Clinical"
          api_name="clinical"
          :max_num="extendCount.Sponsor1Count"
          :defaultFiled="exportDefaultFiled"
          outdataAction="sponsor1group"
        ></Export>
      </div>
      <el-table
        :data="res4"
      ...
      >
                ...
                >
                  <template slot-scope="scope">
                    <div :title="scope.row[item.prop]">
                      <!-- 疾病领域 -->
                      <template v-if="item.prop === 'sponsor2'">
        ...
        >
          <template slot-scope="scope">
            <div :title="scope.row[item.prop]">
              <!-- 疾病领域 -->
              <template v-if="item.prop === 'sponsor1_guifan'">
                <span
      ...
      <div style="positon: relative">
        <el-pagination
          v-if="extendCount.Sponsor1Count"
          ...
          :total="extendCount.Sponsor1Count"
          :disabled="nopms.fy"
          :class="{ 'abandon-click-method': nopms.fy }"
        >
        </el-pagination>
...
const defaultFiled = tablehead.clinical.spz;
const expandFiled = tablehead.clinical.spzExpand;
export default {
  components: {
    LoadingGif,
    List,
  ...
  computed: {
    ...
    ...mapState({
      ...
      res4: (state) =>
        _.keys(state.Clinical.res4).length ? state.Clinical.res4 : [],
      ...
      param: (state) => state.Clinical.sbz_param, // 附加参数(存于vuex内)
  ...
  methods: {
    // 点击跳转到默认列表页并激活参数
    async goListOne(name, num, prop, sponsor2) {
      console.log(name, num, prop, sponsor2);
      if (Number(num) <= 0) return;
      let groups = _.cloneDeep(this.groups);
      // 获取之前的条件筛选选中项
      if (Object.keys(groups).length > 0) {
        groups.phases = this.phaseObj[prop];
        if (groups.sponsor1_guifan) {
          if (!groups.sponsor1_guifan.includes(name)) {
            groups.sponsor1_guifan.push(name);
          }
        } else {
          groups.sponsor1_guifan = [];
          groups.sponsor1_guifan.push(name);
        }
      } else {
        groups.phases = [];
    ...
    handleRequest(props) {
      Store.dispatch("Clinical/getGroupRes", {
        path: "/api/clinical/sponsor2group",
    ...
    getExpandRes(groupname, name, scope) {
      ...
      if (groupname == "sponsor1_guifan") {
        this.expandFiled[0].label = name
        this.expandParams.sponsor1_guifan = scope.row.sponsor1_guifan;
    ...
    toggleRowExpansion(row, column, cell, event) {
      ...
      if (event.target.localName === "td" || event.target.className === "cell")
        return;
      if (column.property == "sponsor1_guifan") {
        let isOneRow = false;
        ...
        if (column.id == this.expands.expanCell && isOneRow) {
          ...
          this.expands.expandsrow.push(row.sponsor1_guifan);
          this.expands.element.removeClass("active");

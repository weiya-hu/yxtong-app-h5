    ...
    <List :loading="listLoading" :nodata="res2 && !res2.length">
      <div class="actions">
        ...
        <Export
          ...
          vuex_name="Clinical"
          api_name="clinical"
          :max_num="extendCount.Conditions1Count"
          :defaultFiled="exportDefaultFiled"
          outdataAction="conditions1group"
        ></Export>
      </div>
      <el-table
        :data="res2"
      ...
      >
                ...
                >
                  <template slot-scope="scope">
                    <div :title="scope.row[item.prop]">
                      <!-- 疾病领域 -->
                      <template v-if="item.prop === 'conditions2'">
                      ...
                      <template v-else>
                        <span
                          ...
                          @click="
                            goListOne(
                              expandParams.conditions1_guifan,
                              scope.row[item.prop],
                              item.label,
                              scope.row.conditions2
                            )
                          "
                        >
                          {{ scope.row[item.prop] }}
        ...
        >
          <template slot-scope="scope">
            <div :title="scope.row[item.prop]">
              <!-- 疾病领域 -->
              <template v-if="item.prop === 'conditions1_guifan'">
                <span
      ...
      <div style="positon: relative">
        <el-pagination
          v-if="extendCount.Conditions1Count"
          ...
          :total="extendCount.Conditions1Count"
          :disabled="nopms.fy"
          :class="{ 'abandon-click-method': nopms.fy }"
        >
        </el-pagination>
...
const defaultFiled = tablehead.clinical.jbly;
const expandFiled = tablehead.clinical.jblyExpand;
export default {
  components: {
    LoadingGif,
    List,
  ...
  computed: {
    ...
    ...mapState({
      ...
      res2: (state) =>
        _.keys(state.Clinical.res2).length ? state.Clinical.res2 : [],
      ...
      param: (state) => state.Clinical.jbly_param, // 附加参数(存于vuex内)
  ...
  methods: {
    // 点击跳转到默认列表页并激活参数
    async goListOne(name, num, prop, conditions2) {
      console.log(name, num, prop, conditions2);
      ...
      //   phases: this.phaseObj[prop],
      //   conditions1_guifan: [name],
      // });
      // if (conditions2) {
      //   query.conditions2 = conditions2
      // }
    ...
    handleRequest(props) {
      Store.dispatch("Clinical/getGroupRes", {
        path: "/api/clinical/conditions2group",
    ...
    getExpandRes(groupname, name, scope) {
      ...
      window.ga(
        ...
        "tab",
        "click",
        this.allBase.dbename + "_conditions1group_" + groupname
      );
      window._paq.push([
        "trackEvent",
        "tab",
      ...
      if (groupname == "conditions1_guifan") {
        this.expandFiled[0].label = name
        this.expandParams.conditions1_guifan = scope.row.conditions1_guifan;
    ...
    toggleRowExpansion(row, column, cell, event) {
      ...
      if (event.target.localName === "td" || event.target.className === "cell")
        return;
      if (column.property == "conditions1_guifan") {
        let isOneRow = false;
        ...
        if (column.id == this.expands.expanCell && isOneRow) {
          ...
          this.expands.expandsrow.push(row.conditions1_guifan);
          this.expands.element.removeClass("active");

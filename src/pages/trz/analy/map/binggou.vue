  ...
  <div class="trz-map-bg">
    ...
    <div class="left">
      <div class="left-top">
        ...
        <div @mouseleave="leaveMap()">
          <div v-show="isWorldMap">
            <!-- 世界地图 -->
            <div id="allChartBg" class="charts"></div>
          </div>

          <div v-show="!isWorldMap">
            <!-- 世界地图 -->
            <div id="chinaChart-bg" class="charts"></div>
          </div>
          ...
          >
            ...
            <div class="company" v-if="companyDetailArr.length > 0">
              <div class="company-title">最新并购事件
              </div>
      ...
      <div class="left-bottom" v-show="isWorldMap">
        ...
        <div class="info">
          <p>并购趋势</p>
          <div id="chart-sj-bg"></div>
        </div>

        <!-- 投资机构 -->
        <div class="info">
          <p>并购企业排行榜（金额）</p>
          <div id="chart-jg-bg"></div>
        </div>
        ...
        <div class="right" v-if="isWorldMap">
          <div class="right-title">各国家并购事件及金额</div>
      ...
      <div class="left-bottom" v-show="!isWorldMap">
        ...
        <!-- 地区事件金额 -->
        <div class="right">
          <div class="right-title">各省市并购事件及金额</div>
          <div class="right-title1">
...
export default {
  ...
  data() {
    return {
      ...
      companyArr: [], // 世界地图底部买方卖方数据
      companyChinaArr: [], // 中国地图底部买方卖方数据
  ...
  beforeDestroy() {
    window.removeEventListener("resize", () => {
      this.echartsResize("chart-sj-bg");
      this.echartsResize("chinachart-sj-bg");
      this.echartsResize("chart-jg-bg");
      this.echartsResize("chinachart-jg-bg");
      this.echartsResize("allChartBg");
      this.echartsResize("chinaChart-bg");
    });
  },
  activated() {
    if (this.isWorldMap) {
  ...
  methods: {
    ...
    getMapData() {
      ...
      window
        .Axios({
          method: "get",
          url: "/api/Tourongzi_analyze_map/bing",
          params: {
        ...
        .then((res) => {
          ...
          if (res.data.code === 200 && res.data.data) {
            ...
            this.drawLine(res.data.data.bot.amount_num, "chart-sj-bg");
            this.drawBar(res.data.data.bot.com_rank, "chart-jg-bg");
            this.companyArr = res.data.data.bot.latest;
            this.defaultOption = {
              step: 0, // 数值越大速度滚动越快
              limitMoveNum: this.isWorldMap
                ? this.worldData.length
                : this.chinaData.length, // 开始无缝滚动的数据量
        ...
        .then((res) => {
          ...
          if (res.data.code === 200 && res.data.data) {
            ...
            this.$nextTick(() => {
              ...
              this.drawLine(res.data.data.bot.amount_num, "chinachart-sj-bg");
              this.drawBar(res.data.data.bot.com_rank, "chinachart-jg-bg");
    ...
    getChartData(name, type) {
      ...
      window
        .Axios({
          method: "get",
          url: "/api/Tourongzi_analyze_map/bingProvince",
          params: {
            accesstoken: GETCOOKIEFUN("accesstoken"),
            type: type,
            name: name,
    ...
    worldConfigure(worldData) {
      ...
      let myChart = Echarts.init(
        document.getElementById("allChartBg"),
        "customed"
      );
      myChart.setOption({
        tooltip: {
    ...
    chinaConfigure(data) {
      ...
      let myChart = Echarts.init(
        document.getElementById("chinaChart-bg"),
        "customed"
      );
    ...
    // 绘制投资机构柱状图
    drawBar(value, id) {
      let data = _.orderBy(value, ["amounts"], ["asc"]),
        that = this,
        yAxis = data.map((item) => item.acquirer),
        series = data.map((item) => item.amounts),
        option = {
          tooltip: {
            trigger: "axis",
            axisPointer: {
      ...
      myChart.on("click", function (params) {
        ...
        if (params.componentType == "yAxis") {
          that.linkQiYeDetail(
            data.filter((item) => item.acquirer === params.value)[0].wyuid
          );
        }
      });
    },

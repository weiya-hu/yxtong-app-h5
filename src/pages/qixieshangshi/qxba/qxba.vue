<template>
  <Main>
    ...
    <el-dialog class="dialog-saved" :visible.sync="savedPopVisible" :modal-append-to-body="false">
      ...
      <SavedPop @closeSavedPop="closeSavedPop" vuex_name="Qxba"></SavedPop>
    </el-dialog>
    ...
    <div slot="left">
      ...
      <!-- 条件筛选（普通/高级搜索之后的二次筛选） -->
      <Filters vuex_name="Qxba"></Filters>
      <!-- 保存的条件 -->
      <Saved vuex_name="Qxba" api_name="qixiebeian"></Saved>
      <!-- 历史搜索 -->
  ...
  export default{
    ...
    computed:{
      ...mapState({
        ...
        nomal_filed: state => state.Qxba.nomal_filed,
        advanced_filed: state => state.Qxba.advanced_filed,
        putong: state => state.Qxba.putong,
        extendCount: state => state.Qxba.extendCount,
        update_conf: state => state.Qxba.update_conf,
      })
    },
    filters: {
      // 给数字每三位数加一个逗号
    ...
    methods:{
      ...
      //获取普通搜索
      nomalSearch () {
        Store.commit("Qxba/conditions",[])
        //清空高级搜索条件和筛选项
        Store.commit('Qxba/groups',{})
        Store.dispatch('Qxba/nomalSearch')
      },
    ...
    mounted () {
      Store.dispatch("Qxba/getBaseInfo").then(() => {
        this.vueTogglePmsTooltip()
      })
      this.vueRouteFrom("Qxba")
    },
  }

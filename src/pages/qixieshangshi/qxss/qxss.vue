<template>
  <Main>
    ...
    <el-dialog class="dialog-saved" :visible.sync="savedPopVisible" :modal-append-to-body="false">
      ...
      <SavedPop @closeSavedPop="closeSavedPop" vuex_name="Qxss"></SavedPop>
    </el-dialog>
    ...
    <div slot="left">
      ...
      <!-- 条件筛选（普通/高级搜索之后的二次筛选） -->
      <Filters vuex_name="Qxss"></Filters>
      <!-- 保存的条件 -->
      <Saved vuex_name="Qxss" api_name="qxss"></Saved>
      <!-- 历史搜索 -->
  ...
  export default{
    ...
    computed:{
      ...mapState({
        ...
        nomal_filed: state => state.Qxss.nomal_filed,
        advanced_filed: state => state.Qxss.advanced_filed,
        putong: state => state.Qxss.putong,
        extendCount: state => state.Qxss.extendCount,
        update_conf: state => state.Qxss.update_conf,
      })
    },
    filters: {
      // 给数字每三位数加一个逗号
    ...
    methods:{
      ...
      //获取普通搜索
      nomalSearch () {
        Store.commit("Qxss/conditions",[])
        //清空高级搜索条件和筛选项
        Store.commit('Qxss/groups',{})
        Store.dispatch('Qxss/nomalSearch')
      },
      // 点击左侧栏中的“高级搜索”触发的事件
      handleAdvancedClick () {
        if (!this.nopms.gjss) {
    ...
    mounted () {
      Store.dispatch("Qxss/getBaseInfo").then(() => {
        this.vueTogglePmsTooltip()
      })
      this.vueRouteFrom("Qxss")
    },
  }

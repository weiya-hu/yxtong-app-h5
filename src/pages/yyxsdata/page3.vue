<template>
  <div class="page-wrap">
    ...
    <div class="top">
      <CascaderSelect
        v-if="atcSel.options.length > 0"
        class="item-sel"
        :placeholder="atcSel.placeholder"
        :option="atcSel.options"
        @updateValue="atcSelChange"
      />
      <Select
        v-model="categSel.model"
        clearable
        :placeholder="categSel.placeholder"
        autoWidth
      >
        <el-option
          v-for="item in categSel.options"
          :key="item"
          :label="item"
          :value="item"
        />
...
export default {
  ...
  methods: {
    ...
    getSearch() {
      this.isloading = true;
      Store.dispatch("Yyxs/getApi", { url: "dlflsearch" }).then((res) => {
          ...
          if (i === 0) {
            ...
            _.forEach(comdata.list, (item, i) => {
              if (this.yearModel && lastData.length > 0) {
                _.forEach(lastData, (items, i) => {
                  if (items.quarter === item.quarter) {
                    item.tb = Number(
                      ((Number(item.sale) - Number(items.sale)) /
                        Number(items.sale)) *
                        100
                    ).toFixed(2);
                  }
    ...
    //柱状折线图
    drawBarLine(data, lastData, xkey, skey, i, isBillion) {

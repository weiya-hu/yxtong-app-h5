<template>
  <Main class="targetdatas">
    ...
    <div slot="left" style="position: relative;">
      ...
      <!-- 热点筛选条件 -->
      <HotFilter vuex_name="Targetdatas" route1="list" route2="analy" route3="qb"></HotFilter>
      <!-- 条件筛选（普通/高级搜索之后的二次筛选） -->
      <Filters vuex_name="Targetdatas" route1="list" route2="analy" route3="qb"></Filters>
      <!-- 保存的条件 -->
      <Saved vuex_name="Targetdatas" api_name="targetdatas"></Saved>
      <!-- 历史搜索 -->
      ...
      <section class="sec2 newsec2">
        <div class="la-body-tab">
          <!-- <div class="market_nav">
            <a @click.prevent="navRouteTo(1)" class="nav_one" :class="{'router-link-active': $route.path == '/targetdatas/list'}">
              <span class="nav"><i class="iconfont pr5" style="font-size:20px;">&#xe62d;</i>注册数据</span>
            </a>
            <a @click.prevent="navRouteTo(2)" class="nav_one" :class="{'router-link-active': $route.path == '/targetdatas/analy', 'abandon-click-method': nopms.ksh}">
  ...
  export default{
    ...
    computed:{
      ...mapState({
        ...
        nomal_filed: state => state.Targetdatas.nomal_filed,
        advanced_filed: state => state.Targetdatas.advanced_filed,
        baseCount: state => state.Targetdatas.extendCount.BaseCount,
        putong: state => state.Targetdatas.putong,
        nopms: state => state.Targetdatas.nopms,
        norules: state => state.Targetdatas.norules,
        update_conf: state => state.Targetdatas.update_conf,
      }),
      // 判断高级搜索旁边的new标签是否显示
      is_show_new () {
    ...
    methods: {
      ...
      //获取普通搜索
      nomalSearch () {
        Store.commit("Targetdatas/conditions",[])
        //清空高级搜索条件和筛选项
        Store.commit('Targetdatas/groups',{})
        if (this.route=='list') {
          Store.dispatch('Targetdatas/nomalSearch')
        }else if(this.route=='analy'){
          Store.dispatch('Targetdatas/getKshRes')
        }
      },
      // 点击左侧栏中的“高级搜索”触发的事件
      handleAdvancedClick () {
      ...
      navRouteTo (index) {
        if (index == 1) {
          this.$router.push({ path: '/targetdatas/list'})
          Store.dispatch("Targetdatas/nomalSearch")
        } else if (index == 2 && !this.nopms.ksh) {
          window.ga("send","event", "tab", "click", "targetdatas_analy");
          window._paq.push(['trackEvent', 'tab', 'click',"targetdatas_analy", ])

          this.$router.push({ path: '/targetdatas/analy'})
          Store.dispatch("Targetdatas/getKshRes")
          window.ga("send","event", "tab", "click", "targetdatas_analy")
          window._paq.push(['trackEvent', 'tab', 'click',"targetdatas_analy", ])
    ...
    created() {
      // Store.commit("Targetdatas/reset")
      setTimeout(() => {
    ...
    mounted () {
      Store.dispatch("Targetdatas/getBaseInfo").then(() => {
        this.vueTogglePmsTooltip()
      })
      this.vueRouteFrom("Targetdatas")
    },
  }
</script>

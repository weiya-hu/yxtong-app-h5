<template>
  <Main>
    ...
    <el-dialog class="dialog-saved" :visible.sync="savedPopVisible" :modal-append-to-body="false">
      ...
      <SavedPop @closeSavedPop="closeSavedPop" vuex_name="ShuomingshuZg"></SavedPop>
    </el-dialog>
    ...
    <el-dialog :visible.sync="dialogFormVisible" :modal-append-to-body="false" width="815px">
      ...
      <Advanced :dialogFormVisible="dialogFormVisible" vuex_name="ShuomingshuZg" api_name="instruct"
    ...
    <div slot="left">
      ...
      <template v-for="(item, index) in vueHandleInitPutong(nomal_filed)">
        <Normal vuex_name="ShuomingshuZg" api_name="instruct" :item="item" :key="index" ref="ileft"></Normal>
      </template>
      <!-- 普通搜索按钮 & 高级搜索入口 -->
      ...
      <!-- 条件筛选（普通/高级搜索之后的二次筛选） -->
      <Filters vuex_name="ShuomingshuZg" route1="instruct" :tooltip="tooltip"></Filters>
      <!-- 保存的条件 -->
      <Saved vuex_name="ShuomingshuZg" api_name="instruct"></Saved>
      <!-- 历史搜索 -->
      ...
      <section class="sec2">
        ...
        <div class="search-body-content">
          <Optioned @showSavedPop="showSavedPop" @clearTheLeft="clearTheLeft" @clearLeft="clearLeft"
            vuex_name="ShuomingshuZg"></Optioned>
...
export default {
  ...
  data() {
    return {
      ...
      tooltip: {
        otcsort: '“其他”分类是除了“甲类”、“乙类”、“按甲类非处方药管理”外的数据集合。',
        update_at: '最新更新说明书，包括了最近一次增加、修改的说明书，不代表说明书最新版本。'
      }
    };
  },
  computed: {
    ...mapState({
      ...
      nomal_filed: state => state.ShuomingshuZg.nomal_filed,
      advanced_filed: state => state.ShuomingshuZg.advanced_filed,
      advanced_filed: state => state.ShuomingshuZg.advanced_filed,
      putong: state => state.ShuomingshuZg.putong,
      extendCount: state => state.ShuomingshuZg.extendCount,
      update_conf: state => state.ShuomingshuZg.update_conf,
    })
  },
  filters: {
    // 给数字每三位数加一个逗号
  ...
  methods: {
    ...
    //获取普通搜索
    nomalSearch() {
      Store.commit("ShuomingshuZg/conditions", [])
      //清空高级搜索条件和筛选项
  ...
  mounted() {
    Store.dispatch("ShuomingshuZg/getBaseInfo").then(() => {
      this.vueTogglePmsTooltip()
    })
    this.vueRouteFrom("ShuomingshuZg")
  },
}
</script>

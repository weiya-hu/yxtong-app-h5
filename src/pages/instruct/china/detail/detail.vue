<template>
  <div class="wrapper instruct" ref="wrapper">
    <!-- 自定义加载界面 -->
          ...
          <div class="list-in-left" :class="{
            ...
            <slide-lan :title="'药品说明书'" class="the-part part-one part-active the-alone">
              <template v-slot:header>
                <div>
                  <div v-if="isEmpty(data_showtuwenbutton) && data_showtuwenbutton.is_show == 2"
                    @click="linkTuWenPage(data_showtuwenbutton)" class="btn">查看图片版本</div>
                </div>
              </template>
            ...
            <slide-lan v-if="other.currentDatas.length > 0" :title="'其他厂家说明书推荐'"
              ...
              <el-table class="other-table" :default-sort="{prop: 'zuixinxiugairi', order: 'descending'}"
                :data="other.currentDatas" style="width: 100%;" header-cell-class-name="header-class"
                cell-class-name="cell-class" border stripe>
...
export default {
  ...
  methods: {
    ...
    linkTuWenPage(showtuwenbutton) {
      // 跳转到图片版本
      if (showtuwenbutton.is_show == 2) {
        this.$router.push({
          path: `/instruct/graphics/${showtuwenbutton.id}`
        })
      }
    },
    linkList(atc) {
  ...
  .detail-list {
    ...
    .main {
      ...
      .right-list {
        ...
        .other-table {
          .cell-class:first-child>.cell>span {
            width: 100%;
          }
        }

<template>
  <Main>
    ...
    <el-dialog class="dialog-saved" :visible.sync="savedPopVisible" :modal-append-to-body="false">
      ...
      <SavedPop @closeSavedPop="closeSavedPop" vuex_name="ShuomingshuRb"></SavedPop>
    </el-dialog>
    ...
    <div slot="left">
      ...
      <!-- 条件筛选（普通/高级搜索之后的二次筛选） -->
      <Filters vuex_name="ShuomingshuRb"></Filters>
      <!-- 保存的条件 -->
      <Saved vuex_name="ShuomingshuRb" api_name="jp_shuomingshu"></Saved>
      <!-- 历史搜索 -->
      ...
      <section class="sec2">
        ...
        <div class="search-body-content">
          <Optioned
            ...
            @clearLeft="clearLeft"
            vuex_name="ShuomingshuRb"></Optioned>
  ...
  export default{
    ...
    computed:{
      ...mapState({
        ...
        allBase: state => state.ShuomingshuRb.allBase,
        helpInfo: state => state.ShuomingshuRb.helpInfo,
        nomal_filed: state => state.ShuomingshuRb.nomal_filed,
        advanced_filed: state => state.ShuomingshuRb.advanced_filed,
        putong: state => state.ShuomingshuRb.putong,
        extendCount: state => state.ShuomingshuRb.extendCount,
        update_conf: state => state.ShuomingshuRb.update_conf,
      })
    },
    filters: {
      // 给数字每三位数加一个逗号
    ...
    methods: {
      ...
      //获取普通搜索
      nomalSearch () {
        Store.commit("ShuomingshuRb/conditions",[])
        //清空高级搜索条件和筛选项
        Store.commit('ShuomingshuRb/groups',{})
    ...
    mounted () {
      Store.dispatch("ShuomingshuRb/getBaseInfo").then(() => {
        this.vueTogglePmsTooltip()
      })
      this.vueRouteFrom("ShuomingshuRb")
    },
  }
</script>

<template>
  <Main>
    ...
    <el-dialog class="dialog-saved" :visible.sync="savedPopVisible" :modal-append-to-body="false">
      ...
      <SavedPop @closeSavedPop="closeSavedPop" vuex_name="ShuomingshuMg"></SavedPop>
    </el-dialog>
    ...
    <div slot="left">
      ...
      <!-- 条件筛选（普通/高级搜索之后的二次筛选） -->
      <Filters vuex_name="ShuomingshuMg"></Filters>
      <!-- 保存的条件 -->
      <Saved vuex_name="ShuomingshuMg" api_name="fdalabel"></Saved>
      <!-- 历史搜索 -->
      ...
      <section class="sec2">
        ...
        <div class="search-body-content">
          <Optioned
            ...
            @clearLeft="clearLeft"
            vuex_name="ShuomingshuMg"></Optioned>
  ...
  export default{
    ...
    computed:{
      ...mapState({
        ...
        allBase: state => state.ShuomingshuMg.allBase,
        helpInfo: state => state.ShuomingshuMg.helpInfo,
        nomal_filed: state => state.ShuomingshuMg.nomal_filed,
        advanced_filed: state => state.ShuomingshuMg.advanced_filed,
        putong: state => state.ShuomingshuMg.putong,
        extendCount: state => state.ShuomingshuMg.extendCount,
        update_conf: state => state.ShuomingshuMg.update_conf,
      })
    },
    filters: {
      // 给数字每三位数加一个逗号
    ...
    methods: {
      ...
      //获取普通搜索
      nomalSearch () {
        Store.commit("ShuomingshuMg/conditions",[])
        //清空高级搜索条件和筛选项
        Store.commit('ShuomingshuMg/groups',{})
    ...
    mounted () {
      Store.dispatch("ShuomingshuMg/getBaseInfo").then(() => {
        this.vueTogglePmsTooltip()
      })
      this.vueRouteFrom("ShuomingshuMg")
    },
  }
</script>

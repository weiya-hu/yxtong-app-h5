<template>
  <Main class="zhuce">
    ...
    <el-dialog class="dialog-saved" :visible.sync="savedPopVisible" :modal-append-to-body="false">
      ...
      <SavedPop @closeSavedPop="closeSavedPop" vuex_name="Linchuangshiyan"></SavedPop>
    </el-dialog>
    ...
    <div slot="left" style="position: relative;">
      ...
      <!-- 热点筛选条件 -->
      <HotFilter vuex_name="Linchuangshiyan" route1="list" route2="analy" route3="qb"></HotFilter>
      <!-- 条件筛选（普通/高级搜索之后的二次筛选） -->
      <Filters vuex_name="Linchuangshiyan" route1="list" route2="analy" route3="qb"></Filters>
      <!-- 保存的条件 -->
      <Saved vuex_name="Linchuangshiyan" api_name="linchuangshiyan"></Saved>
      <!-- 历史搜索 -->
      ...
      <section class="sec2 newsec2">
        <!-- <div class="la-body-tab">
          <div class="market_nav">
            <a @click.prevent="navRouteTo(1)" class="nav_one" :class="{'router-link-active': $route.path == '/linchuangshiyan/list'}">
              <span class="nav"><i class="iconfont pr5" style="font-size:20px;">&#xe62d;</i>中国临床试验</span>
            </a>
            <a @click.prevent="navRouteTo(2)" class="nav_one" :class="{'router-link-active': $route.path == '/linchuangshiyan/analy', 'abandon-click-method': nopms.ksh}">
  ...
  export default{
    ...
    computed:{
      ...mapState({
        ...
        nomal_filed: state => state.Linchuangshiyan.nomal_filed,
        advanced_filed: state => state.Linchuangshiyan.advanced_filed,
        baseCount: state => state.Linchuangshiyan.extendCount.BaseCount,
        putong: state => state.Linchuangshiyan.putong,
        nopms: state => state.Linchuangshiyan.nopms,
        norules: state => state.Linchuangshiyan.norules,
        update_conf: state => state.Linchuangshiyan.update_conf,
      }),
    ...
    methods: {
      ...
      //获取普通搜索
      nomalSearch () {
        Store.commit("Linchuangshiyan/conditions",[])
        //清空高级搜索条件和筛选项
        Store.commit('Linchuangshiyan/groups',{})
        if (this.route=='list') {
      ...
      handleAdvancedClick () {
        ...
        window._paq.push(['trackEvent', 'button', 'click',"advanced_search", "search"])
      ...
      navRouteTo (index) {
        if (index == 1) {
          this.$router.push({ path: '/linchuangshiyan/list'})
    ...
    created() {
      // Store.commit("Linchuangshiyan/reset")
      setTimeout(() => {
    ...
    mounted () {
      Store.dispatch("Linchuangshiyan/getBaseInfo").then(() => {
        this.vueTogglePmsTooltip()
      })
      this.vueRouteFrom("Linchuangshiyan")
    },
  }
</script>

    ...
    <List :loading="listLoading" :nodata="res2 && !res2.length">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div :title="scope.row[item.prop]">
              <div v-if="item.prop === 'name_guifan'" :class="{'extensible-td': Number(scope.row.name_guifan)}">
                <!-- <router-link tag="a" :to="'/search?comprehensive=drug&searchwords=' + scope.row.name" target="_blank"> -->
                  {{ scope.row.name_guifan }}
                <!-- </router-link> -->
              </div>
              <div v-else :class="{'extensible-td': Number(scope.row[item.prop])}" @click="getExpandRes(scope.row.name_guifan, item.prop, scope.row[item.prop],scope)">
                  {{ scope.row[item.prop] }}
              </div>
            </div>
          </template>
...
<script>
  ...
  const defaultFiled = tablehead.linchuangshiyan.ypmc
  const expandFiled = tablehead.linchuangshiyan.ypmcExpand
  export default{
    components: {
      LoadingGif,
      List,
    ...
    computed: {
      ...
      ...mapState({
        ...
        res2: state => _.keys(state.Linchuangshiyan.res2).length ? state.Linchuangshiyan.res2 : [],
        ...
        param: state => state.Linchuangshiyan.ypmc_param, // 附加参数(存于vuex内)
    ...
    methods: {
      ...
      handleRequest(props){
        Store.dispatch("Linchuangshiyan/getGroupRes",{path:'/api/linchuangshiyan/namelist',params:this.expandParams}).then(res=>{
      ...
      sortChange (column, prop, order) {
        ...
        Store.commit('Linchuangshiyan/order', Object.assign({}, this.$store.state.Linchuangshiyan.order, {name: sort ? sort : null}))
        Store.dispatch('Linchuangshiyan/nomalSearch',{params:this.param})
      },
      // 获取展开列表的数据
      getExpandRes (drugsname,groupname,num,scope) {
        this.expandParams.column_name = groupname //用于统计
        ...
        if (Number(num)) {
          this.expandParams.name_guifan_eq = drugsname
      ...
      toggleRowExpansion(row, column, cell, event){
        ...
        if (num) {
          ...
          if (column.id == this.expands.expanCell && isOneRow) {
            ...
            this.expands.expandsrow.push(row.name_guifan)
            this.expands.element.removeClass('active')

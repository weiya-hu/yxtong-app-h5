        ...
        v-for="item in tableHead"
          ...
          <!-- <template :slot="item.tips ? 'header':''" slot-scope="scope">
            <span>
              ...
              <el-tooltip class="item" effect="zhuce" placement="right">
                <div slot="content" v-if="item.prop == 'qiyemingcheng'">{{helpDesc.setList}}</div>
          ...
          <template slot-scope="scope">
            <div :title="scope.row[item.prop]">
              ...
              <div v-else :class="{'extensible-td': Number(scope.row[item.prop])}" @click="getExpandRes(scope.row.qiyemingcheng, item.prop, scope.row[item.prop],scope)">
                {{ scope.row[item.prop] }}
              </div>
            </div>
          </template>
...
<script>
  ...
  const defaultFiled = tablehead.linchuangshiyan.qy
  const expandFiled = tablehead.linchuangshiyan.qyExpand
  ...
  export default {
    ...
    computed: {
      ...
      ...mapState({
        ...
        res3: state => _.keys(state.Linchuangshiyan.res3).length ? state.Linchuangshiyan.res3 : [],
        ...
        param: state => state.Linchuangshiyan.qy_param, // 附加参数(存于vuex内)
    ...
    methods: {
      ...
      handleRequest(props){
        Store.dispatch("Linchuangshiyan/getGroupRes",{path:'/api/linchuangshiyan/qiyelist',params:this.expandParams}).then(res=>{
      ...
      sortChange (column, prop, order) {
        ...
        Store.commit('Linchuangshiyan/order', Object.assign({}, this.$store.state.Linchuangshiyan.order, {qiye: sort ? sort : null}))
        Store.dispatch('Linchuangshiyan/nomalSearch',{params:this.param})
      },
      getExpandRes (qiyemingcheng,groupname,num,scope) {
        ...
        if (Number(num)) {
          this.expandParams.qiyemingcheng_eq = qiyemingcheng
      ...
      toggleRowExpansion(row, column, cell, event){
        ...
        if (num) {
          ...
          if (column.id == this.expands.expanCell && isOneRow) {
            ...
            this.expands.expandsrow.push(row.qiyemingcheng)
            this.expands.element.removeClass('active')

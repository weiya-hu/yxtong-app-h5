<template>
  <Main class="newdrug">
    ...
    <div slot="left" style="position: relative;">
      ...
      <!-- 热点筛选条件 -->
      <HotFilter vuex_name="Newdrug" route1="list" route2="analy" route3="qb"></HotFilter>
      <!-- 条件筛选（普通/高级搜索之后的二次筛选） -->
      <Filters vuex_name="Newdrug" route1="list" route2="analy" route3="qb"></Filters>
      <!-- 保存的条件 -->
      <Saved vuex_name="Newdrug" api_name="newdrug"></Saved>
      <!-- 历史搜索 -->
  ...
  export default{
    ...
    computed:{
      ...mapState({
        ...
        nomal_filed: state => state.Newdrug.nomal_filed,
        advanced_filed: state => state.Newdrug.advanced_filed,
        baseCount: state => state.Newdrug.extendCount.BaseCount,
        putong: state => state.Newdrug.putong,
        nopms: state => state.Newdrug.nopms,
        norules: state => state.Newdrug.norules,
        update_conf: state => state.Newdrug.update_conf,
      })
    },
    filters: {
      // 给数字每三位数加一个逗号
    ...
    methods: {
      ...
      //获取普通搜索
      nomalSearch () {
        Store.commit("Newdrug/conditions",[])
        //清空高级搜索条件和筛选项
        Store.commit('Newdrug/groups',{})
        if (this.route=='list') {
          Store.dispatch('Newdrug/nomalSearch')
        }else if(this.route=='analy'){
          Store.dispatch('Newdrug/getKshRes')
        }
      },
      // 点击左侧栏中的“高级搜索”触发的事件
      handleAdvancedClick () {
      ...
      navRouteTo (index) {
        if (index == 1) {
          ...
          window._paq.push(['trackEvent', 'tab', 'click','newdrug_analy',])

          this.$router.push({ path: '/newdrug/analy'})
          Store.dispatch("Newdrug/getKshRes")
          window.ga("send","event", "tab", "click", "newdrug_analy")
          window._paq.push(['trackEvent', 'tab', 'click','newdrug_analy',])
    ...
    mounted () {
      Store.dispatch("Newdrug/getBaseInfo").then(() => {
        this.vueTogglePmsTooltip()
      })
      this.vueRouteFrom("Newdrug")
    },
  }
</script>

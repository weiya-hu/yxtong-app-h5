<template>
  <Main>
    ...
    <div slot="left">
      ...
      <!-- 条件筛选（普通/高级搜索之后的二次筛选） -->
      <Filters vuex_name="Dae"></Filters>
      <!-- 保存的条件 -->
      <Saved vuex_name="Dae" api_name="dae"></Saved>
      <!-- 历史搜索 -->
  ...
  export default{
    ...
    computed:{
      ...mapState({
        ...
        nomal_filed: state => state.Dae.nomal_filed,
        advanced_filed: state => state.Dae.advanced_filed,
        putong: state => state.Dae.putong,
        extendCount: state => state.Dae.extendCount,
        update_conf: state => state.Dae.update_conf,
      })
    },
    filters: {
      // 给数字每三位数加一个逗号
    ...
    methods:{
      //获取普通搜索
      nomalSearch () {
        Store.commit("Dae/conditions",[])
        //清空高级搜索条件和筛选项
        Store.commit('Dae/groups',{})
        Store.dispatch('Dae/nomalSearch')
      },
      // 点击左侧栏中的“高级搜索”触发的事件
      handleAdvancedClick () {
        if (!this.nopms.gjss) {
    ...
    mounted () {
      Store.dispatch("Dae/getBaseInfo").then(() => {
        this.vueTogglePmsTooltip()
      })
      this.vueRouteFrom("Dae")
    },
  }

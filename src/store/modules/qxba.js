...
export default {
  namespaced: true,
    ...
    error: 0, // 错误码
    path: '/api/qixiebeian', // 维护当前搜索请求接口
    nomal_filed: [], // 普通搜索列举项
    ...
    match_gaoji: {},
    match_shaixuan: Lang.fields.qxba.filters,
  ...
  actions: {
    ...
    nomalSearch(state, Payload) {
      ...
      this.vue.getSearchParams('qixiebeian',state,param)
      return window.Axios({
        method: 'get',
        ...
        if (res.data.code === 200 && res.data) {
          ...
          if (res.data.data.List.count || res.data.data.List.count === 0) {
            state.commit("extendCount", res.data.data.List.count)
          } else {
            // 显示数值归零
            state.commit("extendCount", 0)
          }

          state.commit("res1", res.data.data.List.res)
    ...
    getFiled(state) {
      return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=qxba'
      }).then(res => {
        if (res.data.code === 200 && res.data) {
          // 普通搜索项
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=qxba',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {

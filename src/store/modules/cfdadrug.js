...
export default {
  ...
  mutations: {
    ...
    resetListParams(state){
      ...
      state.qy_param.page = 1
      state.qy_param.order = null
      state.name_param.page = 1
      state.name_param.order = null
    }
  },
  actions: {
    // 普通搜索 和 高级搜索 通用方法
    nomalSearch(state, Payload) {
      ...
      if (!Payload||Payload===state.state.putong){
        ...
        !this.vue.isEqual('cfdadrug',param,false) && state.dispatch("getKshRes",{noClear:true})
      }
      //关键词统计方法
      ...
      if(state.state.tabname==='nameGroup'){
        param = _.assign(param,state.state.name_param)
      }else if(state.state.tabname==='qiyeNameGroup'){
        param = _.assign(param,state.state.qy_param)
      }
      console.log(param);
      // 当存在排序的时候 不去调用条件筛选接口
      if (param.order == null) {
        this.vue.getSearchParams('cfdadrug',state,param,null,Payload)
      }
      ...
      return window.Axios({
        ...
        if (res.data.code === 200 && res.data) {
          ...
          let dataLen = _.keys(res_data).length
          state.commit("error", 0)
          //
          // if (res_data.BaseCount && res_data.BaseCount.shoulihao_count) {
          //   state.commit("extendCount", res_data.BaseCount)
          // } else {
          ...
          //二级列表数据关联到一级列表数据
          let addSecData = res_data.List
          addSecData.res = addSecData.res ? addSecData.res.map((item)=>{
            item.second = {}
            return item
          }) : []
          // 千分位配置设置开始（千分位举例：123,456每三位逗号隔开）
          await this.vue.thousandSet(addSecData.res, 'CfdaDrug').then(value=>{
            addSecData.res = value
            console.log(value)
          })
          // 千分位配置设置结束
    ...
    getKshResMethods(state, Payload) {
      ...
      if(!state.state.kshRes.noClear&&!Payload&&!this.vue.isEqual('cfdadrug',param,false)){
        state.commit("resetListParams")
      }
      ...
      if(Payload) {
        ...
        this.vue.getSearchParams('cfdadrug',state,param)
      }
      return window.Axios({
        method: 'get',
        url: '/api/cfdadrug/visualization',
        params: _.assign(param, {
          accesstoken: GETCOOKIEFUN("accesstoken")
        })
      }).then(res => {
    ...
    getFiled(state) {
       return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=cfdadrug'
      }).then(res => {
        if (res.data.code === 200 && res.data) {
          // 普通搜索项
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=cfdadrug',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {

...
const stateObj = {
  ...
  analyLoading: true,
  error: 0, // 错误码
  tabname: 'basegroup', // 表示目前处于按那种方式浏览/first（受理号）/namegroup（药品名称）/qiyegroup（企业名称）
  path: '/api/linchuangshiyan', // 维护当前搜索请求接口
  //普通查询
...
export default {
  ...
  actions: {
    ...
    nomalSearch(state, Payload) { //type为1时是为了
      ...
      if (state.state.gaoji.words) {
        param = (Payload && Payload.params) ? _.assign({}, sfourth, swords, sgroup, shot, Payload.params) : _.assign({}, sfourth, swords, sgroup, shot, {
          page: 1,
          pageSize: state.state[(state.state.tabname === 'basegroup' ? 'slh' : state.state.tabname === 'namegroup' ? 'ypmc' : 'qy') + '_param'].pageSize
        })
      } else {
      ...
      if (!Payload || Payload === state.state.putong) {
        !this.vue.isEqual('linchuangshiyan', param, false) && state.commit('kshRes', {
          noClear: true
        })
        state.commit('res1', [])
        state.commit('res2', [])
      ...
      return window.Axios({
        method: 'get',
        url: state.state.path === '/api/linchuangshiyan' ? '/api/linchuangshiyan/basegroup' : state.state.path,
        ...
        if (res.data.data) {
          ...
          await this.vue.thousandSet(data.List.res, 'Linchuangshiyan').then(value=>{
            data.List.res = value
            console.log(value)
          })
          // 千分位配置设置结束
        ...
        if (res.data.code === 404) {
          let data = res.data.data
          // if (data.ExtendCount && data.ExtendCount.BaseCount || data.ExtendCount.NameCount  || data.ExtendCount.QiyeCount  || data.ExtendCount.SyjgCount) {
          //   state.commit('extendCount', data.ExtendCount)
          // } else {
          //   state.commit('extendCount',{
          //     BaseCount: 0,
    ...
    getFiled(state) {
      return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=linchuangshiyan'
      }).then(res => {
    ...
    getKshRes(state) {
      ...
      if (!state.state.kshRes.noClear && !this.vue.isEqual('linchuangshiyan', param, false)) {
        state.commit('res1', [])
        state.commit('res2', [])
        state.commit('res3', [])
        state.commit('res4', [])
      ...
      this.vue.getSearchParams('linchuangshiyan', state, param, 'visualization')
      return window.Axios({
        method: 'get',
        url: '/api/linchuangshiyan/visualization',
        params: _.assign(param, {
          accesstoken: GETCOOKIEFUN("accesstoken")
        })
      }).then(res => {
        if (res.data.code === 200 && res.data.data) {
          ...
          // state.commit('extendCount', {
          //   BaseCount: data.aggs.me_id ? data.aggs.me_id : 0,
          //   NameCount: 0,
          //   QiyeCount: 0,
          //   SyjgCount: 0
          // })

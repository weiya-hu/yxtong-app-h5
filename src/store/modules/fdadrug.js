...
export default {
  namespaced: true,
    ...
    //   'tecode': '等效性代码',
    // },
    match_shaixuan: Lang.fields.fdaDrug.filters,
  ...
  actions: {
    ...
    nomalSearch(state, Payload) {
      console.log(state.state.fourth_checked)
      ...
      this.vue.getSearchParams('fdadrug',state,param)
      return window.Axios({
        method: 'get',
    ...
    getFiled(state) {
      return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=fdadrug'
      }).then(res => {
        if (res.data.code === 200 && res.data) {
          // 普通搜索项
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=fdadrug',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {

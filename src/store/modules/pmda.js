...
export default {
  ...
  mutations: {
    ...
    search_api_param(state, Payload) {
      state.search_api_param = Payload
    },
  ...
  actions: {
    ...
    nomalSearch(state, Payload) {
      ...
      this.vue.getSearchParams('pmda',state,param)
      return window.Axios({
        method: 'get',
    ...
    getFiled(state) {
      return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=pmda'
      }).then(res => {
        if (res.data.code === 200 && res.data) {
          // 普通搜索项
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=pmda',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {

...
const stateObj = {
  ...
  analyLoading: true,
  error: 0, // 错误码
  tabname: 'first', // 表示目前处于按那种方式浏览/first（按产品编号浏览）/namegroup（活性成分）/holdergroup（按上市许可持有人浏览）
  path: '/api/epyp', // 维护当前搜索请求接口
  //普通查询
  ...
  holder_param: { // 按按上市许可持有人浏览查询页的附加参数
    order: null, // 排序
    page: 1, // 页数
    pageSize: 20 // 每页条数
  },
  epypzhuce_param: { // 正在审评查询页的附加参数
    order: null, // 排序
    page: 1, // 页数
    pageSize: 20 // 每页条数
  },
  ...
  actions: {
    ...
    nomalSearch (state, Payload) {//type为1时是为了
      ...
      if (state.state.gaoji.words) {
        param = (Payload && Payload.params) ? _.assign({}, sfourth, swords, sgroup, shot, Payload.params) : _.assign({}, sfourth, swords, sgroup, shot, {page: 1, pageSize: state.state[(state.state.tabname==='first'?'slh':state.state.tabname==='namegroup'?'ypmc':state.state.tabname==='holdergroup'?'holder':'epypzhuce')+'_param'].pageSize})
      } else {
        param = (Payload && Payload.params) ? _.assign({}, sfourth, sparam, sgroup, shot, Payload.params) : _.assign({}, sfourth, sparam, sgroup, shot, {page: 1, pageSize: state.state[(state.state.tabname==='first'?'slh':state.state.tabname==='namegroup'?'ypmc':state.state.tabname==='holdergroup'?'holder':'epypzhuce')+'_param'].pageSize})
      }
      ...
      if (!Payload||Payload===state.state.putong) {
        ...
        //   QiyeCount: 0
        // })
        !this.vue.isEqual('epyp',param,false) && state.dispatch("getKshRes",{noClear:true})
        state.commit('res1', [])
        state.commit('res2', [])
        state.commit('res3', [])
        state.commit('res4', [])
        ...
        state.state.holder_param.page = 1
        state.state.holder_param.order = null
        state.state.epypzhuce_param.page = 1
        state.state.epypzhuce_param.order = null
      }
      ...
      this.vue.getSearchParams('epyp',state,param,null,Payload)
      ...
      return window.Axios({
        ...
        if (res.data.data) {
          ...
          console.log(state.state.tabname);
    ...
    getFiled (state) {
      return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=epyp'
      }).then(res => {
    ...
    getKshResMethods (state, Payload) {
      ...
      if(!state.state.kshRes.noClear&&!this.vue.isEqual('epyp',param,false)){
        state.commit('res1', [])
        state.commit('res2', [])
        state.commit('res3', [])
        state.commit('res4', [])
      ...
      if(Payload) {
        ...
        this.vue.getSearchParams('epyp',state,param)
      }
      // this.vue.getSearchParams('epyp',state,param,'visualization')
      return window.Axios({
        method: 'get',
        url: '/api/epyp/visualization',
        params: _.assign(param, {
          accesstoken: GETCOOKIEFUN("accesstoken")
        })
      }).then(res => {
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=epyp',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {
        if (res.data.code === 200 && res.data.data) {
          ...
          for (let item of state.state.norules.exclusive) {
            switch (item.action) {
              ...
              case 'holdergroup': state.state.nopms.holdergroup = true; break
              case 'epypzhuce': state.state.nopms.epypzhuce = true; break

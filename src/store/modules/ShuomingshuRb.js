...
export default {
  namespaced: true,
    ...
    error: 0, // 错误码
    path: '/api/jp_shuomingshu', // 维护当前搜索请求接口
    nomal_filed: [], // 普通搜索列举项
    ...
    //   'fenlei': '药物类型',
    // },
    match_shaixuan: Lang.fields.ShuomingshuRb.filters,
  ...
  actions: {
    ...
    nomalSearch(state, Payload) {
      ...
      if (state.state.gaoji.words) {
        param = (Payload && Payload.params) ? _.assign({}, sfourth, swords, sgroup, Payload.params, state.state.param) : _.assign({}, sfourth, swords, sgroup, {page: 1, pageSize: state.state.param.pageSize})
      } else {
        param = (Payload && Payload.params) ? _.assign({}, sfourth, sparam, sgroup, Payload.params, state.state.param) : _.assign({}, sfourth, sparam, sgroup, {page: 1, pageSize: state.state.param.pageSize})
      }
      //关键词统计方法
    ...
    getFiled(state) {
      return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=jp_shuomingshu'
      }).then(res => {
        if (res.data.code === 200 && res.data) {
          // 普通搜索项
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=jp_shuomingshu',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {

...
export default {
  namespaced: true,
    ...
    //   'gllb': '管理类别',
    // },
    match_shaixuan: Lang.fields.Dae.filters,
  ...
  actions: {
    ...
    nomalSearch(state, Payload) {
      ...
      this.vue.getSearchParams('dae',state,param)
      return window.Axios({
        method: 'get',
    ...
    getFiled(state) {
      return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=dae'
      }).then(res => {
        if (res.data.code === 200 && res.data) {
          // 普通搜索项
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=dae',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {

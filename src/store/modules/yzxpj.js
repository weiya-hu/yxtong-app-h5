  ...
  actions: {
    ...
    nomalSearch(state, Payload) {
      ...
      if (state.state.gaoji.words) {
        param = (Payload && Payload.params) ? _.assign({}, sfourth, swords, sgroup, Payload.params) : _.assign({}, sfourth, swords, sgroup, {page: 1, pageSize: 20})
      } else {
        param = (Payload && Payload.params) ? _.assign({}, sfourth, sparam, sgroup, Payload.params) : _.assign({}, sfourth, sparam, sgroup, {page: 1, pageSize: 20})
      }
    ...
    getFiled(state) {
      return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=yzxjd'
      }).then(res => {
        if (res.data.code === 200 && res.data) {
          // 普通搜索项
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=yzxjd',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {
        if (res.data.code === 200 && res.data.data) {
          ...
          for (let item of state.state.norules.exclusive) {
            switch (item.action)
            {
              ...
              case 'nametimeline': state.state.nopms.nametimeline = true; break;
              case 'companytimeline': state.state.nopms.companytimeline = true; break;
            }
          }

  ...
  actions: {
    ...
    nomalSearch(state, Payload) {
      ...
      if (!Payload||Payload===state.state.putong){
        ...
        !this.vue.isEqual('faers',param,false) && state.dispatch("getKshRes",{noClear:true})
      }
      //关键词统计方法
      ...
      this.vue.getSearchParams('faers',state,param,null,Payload)
      return window.Axios({
        method: 'get',
    ...
    getKshResMethods(state, Payload) {
      ...
      if(!state.state.kshRes.noClear&&!Payload&&!this.vue.isEqual('faers',param,false)){
        state.commit("resetListParams")
      }
      ...
      if(Payload) {
        ...
        this.vue.getSearchParams('faers',state,param)
      }
      return window.Axios({
        method: 'get',
        url: '/api/faers/visualization',
        params: _.assign(param, {
          accesstoken: GETCOOKIEFUN("accesstoken")
        })
      }).then(res => {
    ...
    getFiled(state) {
       return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=faers'
      }).then(res => {
        if (res.data.code === 200 && res.data) {
          // 普通搜索项
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=faers',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {

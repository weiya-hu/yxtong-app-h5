...
const stateObj = {
  ...
  path: '/api/targetdatas', // 维护当前搜索请求接口
  //普通查询
  ...
  match_gaoji: {},
  match_shaixuan: Lang.fields.targetdatas.filters,
...
export default {
  ...
  actions: {
    ...
    nomalSearch (state, Payload) {//type为1时是为了
      ...
      this.vue.getSearchParams('targetdatas',state,param)
      return window.Axios({
        method: 'get',
    ...
    getFiled (state) {
      return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=targetdatas'
      }).then(res => {
    ...
    getKshRes (state) {
      ...
      return window.Axios({
        method: 'get',
        url: '/api/targetdatas/visualization',
        params: _.assign(param, {
          accesstoken: GETCOOKIEFUN("accesstoken")
        })
      }).then(res => {
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=targetdatas',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {

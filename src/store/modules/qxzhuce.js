...
export default {
  ...
  state: {
    ...
    analyLoading: true,
    error: 0, // 错误码
    path: '/api/qxzhuce', // 维护当前搜索请求接口
    nomal_filed: [], // 普通搜索列举项
    advanced_filed: [], // 高级搜索列举项
    extendCount: 0, // 列表数据数量
    res1: {}, // 列表api数据
    ...
    match_gaoji: {},
    match_shaixuan: Lang.fields.qxzhuce.filters,
  ...
  actions: {
    ...
    nomalSearch(state, Payload) {
      ...
      this.vue.getSearchParams('qxzhuce',state,param)
      return window.Axios({
        method: 'get',
    ...
    getFiled(state) {
      return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=qxzhuce'
      }).then(res => {
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=qxzhuce',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {

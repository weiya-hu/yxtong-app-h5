...
export default {
  ...
  state: {
    ...
    error: 0, // 错误码
    path: '/api/orangebook', // 维护当前搜索请求接口
    nomal_filed: [], // 普通搜索列举项
  ...
  actions: {
    ...
    nomalSearch(state, Payload) {
      ...
      this.vue.getSearchParams('orangebook', state, param)
    ...
    getFiled(state) {
      return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=orangebook'
      }).then(res => {
        if (res.data.code === 200 && res.data) {
          // 普通搜索项
    ...
    getBaseInfo(state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=orangebook',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {

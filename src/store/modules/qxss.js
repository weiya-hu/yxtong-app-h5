...
export default {
  namespaced: true,
    ...
    //   'gllb': '管理类别',
    // },
    match_shaixuan: Lang.fields.qxss.filters,
  ...
  mutations: {
    ...
    query(state, Payload) {
      // console.log("默认状态：", state)
      // console.log("默认值：",Payload)
      // opt=1增加条件 或 替换已有条件
  ...
  actions: {
    ...
    nomalSearch(state, Payload) {
      ...
      this.vue.getSearchParams('qxss',state,param)
    ...
    getFiled(state) {
      return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=qxss'
      }).then(res => {
        if (res.data.code === 200 && res.data) {
          // 普通搜索项
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=qxss',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {

...
const stateObj = {
    ...
    error: 0, // 错误码
    path: '/api/binggou', // 维护当前搜索请求接口
    nomal_filed: [{
        field: "acquirer|shortname|applicationscenarios|concept",
        label: "关键词检索",
        name: "keywords",
        placeholder: "融资方、投资方、标签关键词等",
        type: "text",
    }, {
        name: "trzType",
        type: 'check',
    ...
    actions: {
        ...
        nomalSearch(state, Payload) {
            ...
            if (!Payload || Payload === state.state.putong) {
                ...
                state.commit("resetListParams")
                    // !this.vue.isEqual('binggou',param,false) && state.dispatch("getKshRes",{noClear:true})
            }
            //关键词统计方法
            ...
            this.vue.getTrzSearchParams('binggou', state, param, null, Payload)
            return window.Axios({
                method: 'get',
        ...
        getFiled(state) {
            ...
            state.commit("match_shaixuan", match_shaixuan_Obj)
                //  return window.Axios({
                //   method: 'get',
                //   url: '/api/config/search?dbname=binggou/jigou'
                // }).then(res => {
                //   if (res.data.code === 200 && res.data) {
                //     // 普通搜索项
                //     let normal
        ...
        getBaseInfo(state, Payload) {
            return window.Axios({
                method: 'get',
                url: '/api/config/view?dbname=binggou',
                params: {
                    accesstoken: GETCOOKIEFUN("accesstoken")
                }
            }).then(res => {

...
const stateObj = {
  ...
  error: 0, // 错误码
  path: '/api/kuajing', // 维护当前搜索请求接口
  nomal_filed:  [{
    field: "transferor|buyer|applicationscenarios|concept",
    label: "关键词检索",
    name: "keywords",
    placeholder: "项目名称、转让方、受让方等",
    type: "text",
  },{
    name: "trzType",
    type: 'check',
  ...
  filters1: [{
    ...
    field: "guifan_therapeuticarea",
    label: "疾病领域",
    name: "guifan_therapeuticarea",
    sort: "101",
    type: "text",
  },{
    field: "type_item",
    ...
    field: "guifan_cooperationtype",
    label: "合作类型",
    name: "guifan_cooperationtype",
    sort: "98",
    type: "text",
  },{
    field: "type",
    ...
    field: "guifan_developmentstatus",
    label: "产品进度",
    name: "guifan_developmentstatus",
    sort: "69",
    type: "text",
  },{
    field: "guifan_totalamount",
    label: "交易规模",
    name: "guifan_totalamount",
    sort: "62",
    type: "text",
  }
  // ,{
  ...
  actions: {
    ...
    nomalSearch(state, Payload) {
      ...
      if (!Payload||Payload===state.state.putong){
        ...
        // !this.vue.isEqual('kuajing',param,false) && state.dispatch("getKshRes",{noClear:true})
      }
      //关键词统计方法
      ...
      if (Object.keys(sparam).length>0) {
        console.log(this.vue.keywordStatistics(sparam,{keywords: "关键词检索",time: '交易日期'}))
        let obj = this.vue.keywordStatistics(sparam,{keywords: "关键词检索",time: '交易日期'})
      ...
      this.vue.getTrzSearchParams('kuajing',state,param,null,Payload)
      return window.Axios({
        method: 'get',
    ...
    getFiled(state) {
      ...
      state.state.filters1.length > 0 && state.state.filters1.forEach(item => {
        match_shaixuan_Obj[item.field] = item.label;
      })
      match_shaixuan_Obj.province = "授权区域/合作企业总部(省份)"
      match_shaixuan_Obj.city = "授权区域/合作企业总部(城市)"
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=kuajing',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {

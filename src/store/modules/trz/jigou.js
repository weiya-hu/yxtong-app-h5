...
const stateObj = {
    ...
    error: 0, // 错误码
    path: "/api/jigoudetails", // 维护当前搜索请求接口
    nomal_filed: [{
            field: "keywords",
            label: "关键词检索",
            name: "keywords",
    ...
    actions: {
        ...
        nomalSearch(state, Payload) {
            ...
            if (!Payload || Payload === state.state.putong) {
                ...
                // !this.vue.isEqual('jigoudetails',param,false) && state.dispatch("getKshRes",{noClear:true})
            }
            //关键词统计方法
            ...
            if (this.vue.vueGetCookie("from_jigoudetails_home")) {
                param["from_jigoudetails_home"] = this.vue.vueGetCookie(
                    "from_jigoudetails_home"
                );
            }
            param = _.assign(param, state.state.param);

            this.vue.getTrzSearchParams("jigoudetails", state, param, null, Payload);
            return window
                .Axios({
                    method: "get",
                ...
                .then(res => {
                    if (this.vue.vueGetCookie("from_jigoudetails_home")) {
                        this.vue.vueDelCookie("from_jigoudetails_home");
                    }
        ...
        getFiled(state) {
            ...
            match_shaixuan_Obj.province = "机构总部(省份)";
            match_shaixuan_Obj.city = "机构总部(城市)";
            console.log(match_shaixuan_Obj);
            state.commit("match_shaixuan", match_shaixuan_Obj);
            //  return window.Axios({
            //   method: 'get',
            //   url: '/api/config/search?dbname=trz/jigou'
            // }).then(res => {
            //   if (res.data.code === 200 && res.data) {
            //     // 普通搜索项
            //     let normal
        ...
        getBaseInfo(state, Payload) {
            return window
                .Axios({
                    method: "get",
                    url: "/api/config/view?dbname=jigoudetails",
                    params: {
                        accesstoken: GETCOOKIEFUN("accesstoken")
                    }
                })

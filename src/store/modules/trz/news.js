...
const stateObj = {
    ...
    error: 0, // 错误码
    path: "/api/tourongzi_news", // 维护当前搜索请求接口
    nomal_filed: [{
            field: "title|digest",
            label: "关键词检索",
            name: "keywords",
            placeholder: "机构企业等关键词",
            type: "text"
        },
        {
            field: "time",
    ...
    actions: {
        ...
        nomalSearch(state, Payload) {
            ...
            if (!Payload || Payload === state.state.putong) {
                ...
                // !this.vue.isEqual('tourongzi_news/index',param,false) && state.dispatch("getKshRes",{noClear:true})
            }
            //关键词统计方法
            ...
            if (this.vue.vueGetCookie("from_news_home")) {
                param["from_news_home"] = this.vue.vueGetCookie(
                    "from_news_home"
                );
            }
            ...
            return window
                ...
                .then(res => {
                    if (this.vue.vueGetCookie("from_news_home")) {
                        this.vue.vueDelCookie("from_news_home");
                    }
        ...
        getFiled(state) {
            ...
            //  return window.Axios({
            //   method: 'get',
            //   url: '/api/config/search?dbname=tourongzi_news'
            // }).then(res => {
            //   if (res.data.code === 200 && res.data) {
            //     // 普通搜索项
            //     let normal
        ...
        getBaseInfo(state, Payload) {
            return window
                .Axios({
                    method: "get",
                    url: "/api/config/view?dbname=tourongzi_news",
                    params: {
                        accesstoken: GETCOOKIEFUN("accesstoken")
                    }
                })

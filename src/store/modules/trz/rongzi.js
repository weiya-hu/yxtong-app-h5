...
const stateObj = {
    ...
    error: 0, // 错误码
    path: "/api/trz", // 维护当前搜索请求接口
    nomal_filed: [{
            field: "shortname|investors|applicationscenarios|concept",
            label: "关键词检索",
            name: "keywords",
    ...
    actions: {
        ...
        nomalSearch(state, Payload) {
            ...
            if (!Payload || Payload === state.state.putong) {
                ...
                // !this.vue.isEqual('trz',param,false) && state.dispatch("getKshRes",{noClear:true})
            }
            //关键词统计方法
            ...
            if (this.vue.vueGetCookie("from_tourongzi_home")) {
                param["from_tourongzi_home"] = this.vue.vueGetCookie(
                    "from_tourongzi_home"
                );
            }
            param = _.assign(param, state.state.param);

            this.vue.getTrzSearchParams("trz", state, param, null, Payload);
            return window
                .Axios({
                    method: "get",
                ...
                .then(res => {
                    if (this.vue.vueGetCookie("from_tourongzi_home")) {
                        this.vue.vueDelCookie("from_tourongzi_home");
                    }
        ...
        getBaseInfo(state, Payload) {
            return window
                .Axios({
                    method: "get",
                    url: "/api/config/view?dbname=trz",
                    params: {
                        accesstoken: GETCOOKIEFUN("accesstoken")
                    }
                })

...
export default {
  namespaced: true,
    ...
    error: 0, // 错误码
    path: '/api/fdaproducts', // 维护当前搜索请求接口
    nomal_filed: [], // 普通搜索列举项
    ...
    //   'tecode': '等效性代码',
    // },
    match_shaixuan: Lang.fields.orangeAmerica.filters,
  ...
  actions: {
    ...
    nomalSearch(state, Payload) {
      ...
      this.vue.getSearchParams('fdaproducts',state,param)
      return window.Axios({
        method: 'get',
    ...
    getFiled(state) {
      return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=fdaproducts'
      }).then(res => {
        if (res.data.code === 200 && res.data) {
          // 普通搜索项
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=fdaproducts',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {

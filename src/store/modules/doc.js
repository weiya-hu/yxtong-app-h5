...
export default {
  namespaced: true,
    ...
    //   'gllb': '管理类别',
    // },
    match_shaixuan: Lang.fields.Doc.filters,
  ...
  actions: {
    ...
    nomalSearch(state, Payload) {
      ...
      this.vue.getSearchParams('doc',state,param)
      return window.Axios({
        method: 'get',
        ...
        if (res.data.code === 200 && res.data) {
          ...
          let resGroupList = res.data.data.GroupList
          state.commit("error", 0)
          if (resList.count || resList.count === 0) {
            state.commit("extendCount", resList.count)
          } else {
            // 显示数值归零
            state.commit("extendCount", 0)
          }

          state.commit("res1", resList.res)
    ...
    getFiled(state) {
      return window.Axios({
        method: 'get',
        url: '/api/config/search?dbname=doc'
      }).then(res => {
        if (res.data.code === 200 && res.data) {
          // 普通搜索项
    ...
    getBaseInfo (state, Payload) {
      return window.Axios({
        method: 'get',
        url: '/api/config/view?dbname=doc',
        params: {
          accesstoken: GETCOOKIEFUN("accesstoken")
        }
      }).then(res => {
